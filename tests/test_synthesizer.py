...
def _plan():
    return ActionPlan(
        source="t",
        task="write a haiku about the sea then count its lines",
        steps=[
...
def _session(*outcomes):
    return RunSession(
        ...
        verification=VerificationResult(
            ok=True, envelope_id="env_1", plan_id="plan_1", duration_ms=1.0
        ),
        steps=list(outcomes),
    )
...
def test_collect_outputs_pairs_output_with_kind():
    outs = collect_outputs(
        _session(_outcome("t1", "sea foam / white crests"), _outcome("s1", "2")), _plan()
    )
...
async def test_synthesize_uses_llm_answer_when_available():
    ...
    res = await synthesize(
        "write a haiku then count lines",
        _session(_outcome("t1", "sea"), _outcome("s1", "2")),
        _plan(),
        client=_FakeClient(answer_obj),
    )
...
async def test_deterministic_fallback_when_llm_disabled():
    res = await synthesize(
        "q",
        _session(_outcome("t1", "OUTPUT-ALPHA"), _outcome("s1", "OUTPUT-BETA")),
        _plan(),
        use_llm=False,
    )
...
async def test_synthesize_falls_back_when_client_raises():
    res = await synthesize(
        "q", _session(_outcome("t1", "KEEP-ME")), _plan(), client=_RaisingClient()
    )
    assert res.used_llm is False

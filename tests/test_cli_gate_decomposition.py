"""`daisugi gate init --allow-shell-decomposition` (ADR-0010).
...
def test_gate_init_help_mentions_the_flag():
    # Rich (pulled in with the [tui] extra) renders help as a table and ellipsis-
    # truncates the long flag name to `--allow-shell-decom…`. Click's CliRunner
    # pins COLUMNS=80, so the width can't be widened from here. Match the visible,
    # un-truncated prefix — it is unique to this flag, so it still proves the flag
    # is documented.
    ...
    assert "--allow-shell-decom" in clean

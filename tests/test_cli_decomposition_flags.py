...
def test_the_flag_is_documented_on_every_envelope_producing_command(argv):
    # Rich (via the [tui] extra) ellipsis-truncates the long flag to
    # `--allow-shell-decom…` in help, and Click's CliRunner pins COLUMNS=80 so the
    # width can't be widened here. Match the visible prefix — unique to this flag.
    res = runner.invoke(app, argv)
    assert res.exit_code == 0, res.output
    import re

    clean = re.sub(r"\x1b\[[0-9;]*m", "", res.output)
    assert _FLAG[: len("--allow-shell-decom")] in clean
...
def test_generate_envelope_resolves_config_from_its_own_data_dir(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("opendaisugi.envelope.generate_envelope", _fake_generate)

...
def test_gate_proposals_lists_nothing_by_default(tmp_path):
    root = tmp_path / "gateroot"
...
def test_gate_proposals_lists_a_recorded_proposal(tmp_path):
    import time

    from opendaisugi.ask import propose
    ...
    res = runner.invoke(app, ["gate", "proposals", "--root", str(root)])
    assert res.exit_code == 0
    assert "widen-allowlist" in res.output
    assert "shell" in res.output

    as_json = runner.invoke(app, ["gate", "proposals", "--root", str(root), "--json"])

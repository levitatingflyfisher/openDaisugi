...
def test_contradiction_invariant_is_hard_error_even_at_low_stakes():
    ...
    env = Envelope(
        ...
        stakes="low",
        invariants=[
            Invariant(type="always_false", description="x", expr=_contradiction(), enforce=True)
        ],
    )
    result = verify(plan, env)
    assert not result.ok
    assert any(
        "never be satisfied" in v.message or "unsatisfiable" in v.message for v in result.violations
    )


def test_tautological_alias_rejected_on_register():
    reg = AliasRegistry()
    taut = parse_expression(
        {
            "op": "or",
            "children": [
                {"op": "equals", "path": "type", "value": "shell"},

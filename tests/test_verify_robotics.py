...
def test_verify_robot_plan_fails_on_workspace_violation():
    ...
    assert any(v.detail.get("invariant") == "end_effector_in_workspace" for v in result.violations)
...
def test_robotics_invariant_without_backing_bounds_is_rejected():
    ...
    assert not r.ok
    assert any(
        "backing" in v.message.lower() or "workspace_bounds" in v.message for v in r.violations
    )
...
def test_robotics_invariant_with_bounds_still_works():
    ...
    inside = ActionPlan(
        source="t", task="x", steps=[CartesianMoveStep(id="m", target_position=(0.5, 0.5, 0.5))]
    )
    assert verify(inside, env).ok
    outside = ActionPlan(
        source="t", task="x", steps=[CartesianMoveStep(id="m", target_position=(9.0, 9.0, 9.0))]
    )

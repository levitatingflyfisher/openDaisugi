...
def test_find_ended_reports_done_with_the_real_exit_code(tmp_path, coppice_server):
    """wait_for_state (registry.py) calls find_ended when a pane is absent
    from the live list, so a wait started after the pane already ended -
    never seen alive during that wait at all - still resolves instead of
    timing out. This is the backend half of that fix, against a real
    coppice server: the pane's own recorded exit code must come through in
    the event's detail, not a guess."""
    backend = CoppiceBackend(sock_path=coppice_server.socket_path, data_dir=coppice_server.data_dir)
    ref = backend.spawn(cwd=tmp_path, cmd=["sh", "-c", "exit 3"], env={}, label="ends", kind="pty")
    backend._call("pane.wait_output", pane=ref.id, state="done", timeout_ms=5000)

    ev = backend.find_ended(ref)

    assert ev is not None
    assert ev.state == "done"
    assert ev.detail == "exit=3"


def test_find_ended_is_none_for_a_live_pane(tmp_path, coppice_server):
    backend = CoppiceBackend(sock_path=coppice_server.socket_path, data_dir=coppice_server.data_dir)
    ref = backend.spawn(
        cwd=tmp_path, cmd=["sh", "-c", "sleep 300"], env={}, label="alive", kind="pty"
    )
    try:
        assert backend.find_ended(ref) is None
    finally:
        backend.close(ref)


def test_find_ended_is_none_for_an_unknown_pane(tmp_path, coppice_server):
    backend = CoppiceBackend(sock_path=coppice_server.socket_path, data_dir=coppice_server.data_dir)
    assert backend.find_ended(PaneRef(backend.name, "w9:p9")) is None


def test_find_ended_and_wait_for_state_resolve_done_after_a_restart(tmp_path, coppice_binary):
    """The reviewer's own repro: a pane that already ended BEFORE a
    restart has no state event in the new server's fresh state store -
    Restore's AlreadyClosed branch only stamps ended_at, it does not (and
    should not) invent a receive-time event for a fact this server never
    itself observed - so the row used to fall back to state unknown with
    no ts, `_state_from_pane_row` (it requires ts) returned None, and
    find_ended returned None right along with it: wait_for_state then
    timed out waiting for done on a pane that had, in fact, already ended.

    This drives a real second server on the first one's own data dir, the
    same way harness/coppice/internal/server/restart_test.go does on the
    Go side, rather than the coppice_server fixture's one long-lived
    server: `server start`/`server stop` always take --socket/--data-dir
    of their own (coppice_sandbox._run_cli), a temp dir this test owns,
    never the operator's default socket or ~/.opendaisugi.
    """
    ...
    def run(*args, allow_autostart=False):
        return coppice_sandbox._run_cli(
            coppice_binary.binary, socket_path, data_dir, *args, allow_autostart=allow_autostart
        )

    proc = run("server", "start", allow_autostart=True)
    assert proc.returncode == 0, proc.stderr

    backend = CoppiceBackend(sock_path=socket_path, data_dir=data_dir)
    ref = backend.spawn(cwd=tmp_path, cmd=["sh", "-c", "exit 7"], env={}, label="ends", kind="pty")
    backend._call("pane.wait_output", pane=ref.id, state="done", timeout_ms=5000)

    stopped = run("server", "stop", allow_autostart=True)
    assert stopped.returncode == 0, stopped.stderr

    started_again = run("server", "start", allow_autostart=True)
    assert started_again.returncode == 0, started_again.stderr
    try:
        backend2 = CoppiceBackend(sock_path=socket_path, data_dir=data_dir)

        ev = backend2.find_ended(ref)
        assert ev is not None, "find_ended returned None for a pane that ended before the restart"
        assert ev.state == "done"
        assert ev.detail == "exit=7"

        resolved = wait_for_state(backend2, ref, until="done", timeout_s=2.0, poll_s=0.05)
        assert resolved is not None, "wait_for_state timed out on a pane that had already ended"
        assert resolved.state == "done"
    finally:
        run("server", "stop", allow_autostart=True)
...
def _git_repo(path: Path) -> Path:
    """A repo on branch main with one commit, or a skip when git is absent."""
...
def test_a_pane_spawned_into_a_worktree_task_runs_there(tmp_path, coppice_server):
    """create_task with a worktree makes the worktree beside the repo; a pane
    spawned with task= reports that directory as its cwd; close_task with
    keep_worktree leaves the directory and closes the pane."""
    repo = _git_repo(tmp_path)
    backend = CoppiceBackend(
        sock_path=coppice_server.socket_path, data_dir=coppice_server.data_dir, timeout_s=2.0
    )
    label = f"wt-{os.getpid()}"
    task = backend.create_task(label, cwd=str(repo), worktree=True)
    assert task.ref.startswith("t")
    assert task.label == label
    assert task.worktree == str(repo) + "-worktrees/" + label
    assert (Path(task.worktree) / ".git").exists()
    assert task.state == ""
    assert task.panes == ()
    ref = backend.spawn(
        cwd=None, cmd=["sh", "-c", "sleep 30"], env={}, label="in-task", kind="pty", task=task.ref
    )
    try:
        cwds = {info.ref.id: info.cwd for info in backend.list()}
        assert cwds[ref.id] == task.worktree
        listed = {t.ref: t for t in backend.list_tasks()}
        assert listed[task.ref].panes == (ref.id,)
        assert listed[task.ref].state in {"working", "idle", "unknown"}
    finally:
        backend.close_task(task.ref, keep_worktree=True)
    assert (Path(task.worktree) / ".git").exists()
    assert task.ref not in {t.ref for t in backend.list_tasks()}
    # close_task closes its panes the way an operator pane.close does
    # (coppice S1): the record is removed at once, not marked closed and
    # kept. It never lands in Recent (`ended: true`) either.
    live_ids = {row["id"] for row in backend._call("pane.list")["panes"]}
    assert ref.id not in live_ids
    ended_ids = {row["id"] for row in backend._call("pane.list", ended=True)["panes"]}
    assert ref.id not in ended_ids


def test_list_tasks_is_empty_when_the_host_is_gone(tmp_path):
    backend = CoppiceBackend(sock_path=tmp_path / "nothing.sock", data_dir=tmp_path)
    assert backend.list_tasks() == []


def test_list_tasks_is_empty_when_the_server_refuses_the_verb(tmp_path, monkeypatch):
    backend = CoppiceBackend(sock_path=tmp_path / "server.sock", data_dir=tmp_path)

    def refuse(cmd, timeout_s=None, **fields):
        raise CoppiceError("bad_request", f'no command "{cmd}"')

    monkeypatch.setattr(backend, "_call", refuse)
    assert backend.list_tasks() == []

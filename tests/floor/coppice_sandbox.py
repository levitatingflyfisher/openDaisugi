...
def _run_cli(
    ...
    """
    env = dict(os.environ)
    # A pane.list row's stack.router comes from these two, which every pane
    # inherits from the server. A shape test must not change with whatever
    # the person running the tests exported, such as a gateway install.
...
def record_pane_list_fixture(server: CoppiceServer) -> dict:
    ...
    `pane.list` answers with every LIVE pane the server currently has (an
    ended one only shows with `ended: true`, and an operator-closed one
    never shows again at all), so this picks out only the row for the pane
    it just created rather than the first row in the reply - a server this
    module shares with other tests in the same run may already have live
    panes of its own ahead of this one.
    """
    created = _raw_call(
        server.socket_path,
        "pane.create",

...
async def test_full_loop_rejection_then_hinted_regeneration(tmp_path, mock_llm_client):
    ...
    with (
        patch("opendaisugi.supervisor.verify", side_effect=_accept_whole_plan),
        patch(
            "opendaisugi.supervisor.verify_step", side_effect=_reject_only_per_step(_real_verify)
        ),
    ):
    ...
    assert mock_llm_client.call_count == 2, (
        "second generate_envelope must hit the LLM (not short-circuit)"
    )

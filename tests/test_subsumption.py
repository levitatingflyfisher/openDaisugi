...
# --- compound-shell decomposition is a capability, so it must be contained -----


def _decomp_env(allowlist, *, decompose: bool):
    return Envelope(
        generated_by="t",
        task="t",
        permissions=Permission(
            shell=True, shell_allowlist=list(allowlist), shell_allow_decomposition=decompose
        ),
    )


def test_inner_cannot_grant_itself_shell_decomposition():
    """The same allowlist admits `a && b` with the opt-in and only `a` without
    it, so an inner envelope that opts in is NOT contained by an outer that
    does not — the delegation/contract counterpart of the inheritance check."""
    outer = _decomp_env(["git", "ls"], decompose=False)
    inner = _decomp_env(["git", "ls"], decompose=True)
    r = envelope_subsumes(outer, inner)
    assert not r.holds
    assert any("shell_allow_decomposition" in reason for reason in r.reasons)


def test_inner_may_decline_a_decomposition_the_outer_allows():
    outer = _decomp_env(["git", "ls"], decompose=True)
    inner = _decomp_env(["git"], decompose=False)
    assert envelope_subsumes(outer, inner).holds


def test_both_opted_in_still_subsumes_on_a_narrower_allowlist():
    outer = _decomp_env(["git", "ls"], decompose=True)
    inner = _decomp_env(["git"], decompose=True)
    assert envelope_subsumes(outer, inner).holds

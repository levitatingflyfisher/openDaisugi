...
Tested over injected callables (no real LLM / filesystem parsing) — the orchestration
...
def test_onboard_warns_loudly_when_every_episode_errors():
    ...
    assert any("envelope" in w.lower() and "configured" in w.lower() for w in report.warnings), (
        report.warnings
    )

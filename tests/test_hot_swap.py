...
def prove_router_swap(tmp: Path) -> None:
    ...
    The behavioural half uses `gateway_local_model` because it exists today.
    The structural half, `test_reloader_hands_the_rebuild_a_whole_fresh_config`
    in tests/test_gateway_reload.py, is what covers a later router field with
    no new wiring: the reloader hands the rebuild a whole fresh Config, not a

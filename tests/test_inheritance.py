...
# --- compound-shell decomposition is a capability, so it inherits like one -----


def test_child_cannot_enable_shell_decomposition_when_parent_forbids_it():
    """Turning decomposition on widens what the shell allowlist admits
    (`a && b` instead of `a`), so a child may not grant itself the opt-in."""
    parent = _env(shell=True, shell_allowlist=["git"], shell_allow_decomposition=False)
    child = _env(shell=True, shell_allowlist=["git"], shell_allow_decomposition=True)
    violations = verify_inheritance(child, parent)
    assert any("shell_allow_decomposition" in v.message for v in violations)


def test_child_may_drop_shell_decomposition_the_parent_allows():

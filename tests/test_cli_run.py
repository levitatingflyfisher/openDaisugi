...
pytestmark = pytest.mark.skipif(sys.platform == "win32", reason="daisugi run is POSIX-only in v0.1")
...
def test_run_yes_flag_sets_always_approve(tmp_path, monkeypatch):
    ...
    plan_path, env_path = _write_plan_and_envelope(tmp_path, "echo cli-test-ok", ["echo"])
    runner = CliRunner()
    result = runner.invoke(
        app,
        [

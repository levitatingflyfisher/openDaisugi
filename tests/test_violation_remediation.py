...
def test_allowlist_miss_suggests_the_minimal_amendment():
    """Head not in allowlist names the exact envelope widening that would fix it
    (reject-with-remedy, ADR-0013) — never a decomposition suggestion."""
    env = _env(["cat"])
    plan = ActionPlan(source="t", task="t", steps=[ShellStep(id="s1", command="rm -rf /tmp/x")])
    violations = check_permissions(plan, env)
    assert violations[0].suggested_remediation == "Add 'rm' to permissions.shell_allowlist"

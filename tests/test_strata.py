...
def _write(path: str) -> ActionPlan:
    return ActionPlan(source="t", task="t", steps=[FileWriteStep(id="w", path=path, content="x")])
...
def test_promote_refuses_an_already_promoted_constraint():
    ...
    second = strata.promote_constraint(first.envelope, con, remove_file_write=["/data/tenantY/**"])

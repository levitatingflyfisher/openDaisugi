...
async def test_zero_steps_raises_no_steps_error():
    result = _dp()  # DecomposedPlan(steps=[])
    with pytest.raises(NoStepsError):
        await decompose("hello", client=_FakeClient(result))
...
async def test_inventory_lists_available_skills_and_tools():
    ...
    await decompose(
        "do a thing",
        client=client,
        available_skills=["tidy-inbox"],
        available_mcp_tools=["github/create_issue"],
    )
    user_msg = client.messages[-1]["content"]

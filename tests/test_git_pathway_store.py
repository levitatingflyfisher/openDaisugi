...
def test_publish_and_pull_roundtrip(tmp_path: Path):
    ...
    subprocess.run(["git", "-C", str(repo_b), "pull"], check=True, capture_output=True)

    a_store = GitPathwayStore(
        repo_path=repo_a,
        private_key_b64=priv_a,
        public_key_b64=pub_a,
        publisher="alice@dev",
    )
...
def test_pull_refuses_bundle_from_untrusted_signer(tmp_path: Path):
    ...
    a_store = GitPathwayStore(
        repo_path=repo_a,
        private_key_b64=priv_alice,
        public_key_b64=pub_alice,
        publisher="alice@dev",
    )
...
def test_offline_ok_tolerates_pull_failure(tmp_path: Path):
    ...
    # Break the remote
    subprocess.run(
        ["git", "-C", str(repo), "remote", "set-url", "origin", "/nonexistent/path/to/nowhere.git"],
        check=True,
    )
...
def test_in_repo_trusted_signers_file_is_not_a_trust_anchor(tmp_path: Path):
    ...
    a_store = GitPathwayStore(
        repo_path=repo_a, private_key_b64=priv_evil, public_key_b64=pub_evil, publisher="evil@dev"
    )
    a_store.publish(_pathway())

...
def test_generate_envelope_prints_yaml_by_default(tmp_path):
    fake = _fake_envelope()
    with patch(
        "opendaisugi.envelope.generate_envelope",
...
def test_generate_envelope_prints_json_with_flag(tmp_path):
    ...
    loaded = json.loads(result.stdout[result.stdout.index("{") :])
    assert loaded["id"] == "env_cli0001"
...
def test_generate_envelope_passes_model_through(tmp_path):
    ...
    with patch("opendaisugi.envelope.generate_envelope", side_effect=fake):
        result = runner.invoke(
            app,
            [
                "generate-envelope",
...
def test_generate_envelope_task_too_long_exits_two(tmp_path):
    ...
    with patch("opendaisugi.envelope.generate_envelope", side_effect=raise_too_long):
...
def test_generate_envelope_llm_error_exits_two(tmp_path):
    ...
    with patch("opendaisugi.envelope.generate_envelope", side_effect=raise_llm_error):
...
def test_journal_search_import_error_goes_to_stderr(tmp_path):
    ...
    with patch.object(journal, "search", side_effect=raise_import):
        with patch("opendaisugi.journal.Journal", return_value=journal):
            result = runner.invoke(
...
def test_journal_ingest_processes_episodes(tmp_path):
    # Envelopes are inferred from observed steps (ADR-0016) — no LLM, no mock.
    episodes_path = _write_episodes_file(tmp_path)
    result = runner.invoke(
        app,
        ["journal", "ingest", str(episodes_path), "--data-dir", str(tmp_path)],
    )
    assert result.exit_code == 0, result.output
...
def test_journal_ingest_dry_run(tmp_path):
    ...
    assert result.exit_code == 0, result.output
    # A dry-run now previews the deterministic verdict (would pass/fail) and
    # writes nothing, rather than emitting a placeholder status.
    assert "dry run" in result.output.lower()
    assert "would pass" in result.output.lower() or "would fail" in result.output.lower()
...
def test_journal_ingest_json_output(tmp_path):
    ...
    result = runner.invoke(
        app,
        ["journal", "ingest", str(episodes_path), "--data-dir", str(tmp_path), "--json"],
    )
    assert result.exit_code == 0, result.output
...
def test_journal_ingest_errored_episode_exits_one(tmp_path):
    """If any episode errors (inference exception), the command exits 1."""
    episodes_path = _write_episodes_file(tmp_path, n_episodes=2)

    def raising_infer(records, *, task="", **kw):
        raise RuntimeError("simulated inference failure")

    with patch("opendaisugi.ingest.infer_envelope", side_effect=raising_infer):
        result = runner.invoke(
            app,
            [
                "journal",
...
def test_version_flag_prints_version_and_exits_zero():
    # clig.dev: ship --version. A top-level `daisugi --version` must work.

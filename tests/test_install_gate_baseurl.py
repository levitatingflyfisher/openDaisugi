...
def test_claude_gate_ask_bakes_the_flag_into_the_installed_hook(tmp_path):
    (tmp_path / ".claude").mkdir()
    install(
        home=tmp_path,
        yes=True,
    ...
    gate_cmds = [c for c in commands if "opendaisugi.gate" in c]
    assert len(gate_cmds) == 1
    assert "--ask" in gate_cmds[0]


def test_claude_gate_without_ask_has_no_ask_flag(tmp_path):
    (tmp_path / ".claude").mkdir()
    install(
        home=tmp_path,
        yes=True,
...
def test_claude_gate_upgrade_rewrites_old_module_to_resident_client(tmp_path):
    """ADR-0017: an install from before the resident gate wrote the old
    `-m opendaisugi.gate` command directly. A plain re-run of
    `daisugi install --gate` must upgrade it to `-m opendaisugi.gate_client`
    in place — not warn-and-skip, which would leave the one real user stuck
    on the slow (~0.7s cold) path forever on every future re-install.
    """
    (tmp_path / ".claude").mkdir()
    ...
    (tmp_path / ".claude" / "settings.json").write_text(json.dumps(pre_existing))
    ...
    commands = _pre_commands(_settings(tmp_path))
    assert any("opendaisugi.gate_client" in c for c in commands)
    assert not any(c == old_command for c in commands)
    assert sum("opendaisugi.gate" in c for c in commands) == 1  # still exactly one gate hook
    assert not any("already installed" in str(w.message) for w in caught)
...
def test_cli_install_ask_flag_reaches_the_installed_hook(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        app,
        ["install", "--gate", "--enforce", "--ask", "--yes", "--runtime", "claude"],
    )
    assert result.exit_code == 0
    commands = _pre_commands(_settings(tmp_path))
    assert any("opendaisugi.gate" in c and "--ask" in c for c in commands)
...
def test_cli_install_ask_without_enforce_is_inert_and_not_baked_in(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        app,
        ["install", "--gate", "--ask", "--yes", "--runtime", "claude"],
    )
    assert result.exit_code == 0
    out = _cli_output(result)
    assert "only applies with --gate --enforce" in out
    commands = _pre_commands(_settings(tmp_path))
    gate_cmds = [c for c in commands if "opendaisugi.gate" in c]
    assert len(gate_cmds) == 1
    assert "--ask" not in gate_cmds[0]

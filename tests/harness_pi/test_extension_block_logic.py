...
@_NODE
def test_report_state_carries_the_callers_coppice_sock_and_pane_on_the_request(tmp_path):
    """The resident gate's own environment names no pane at all (it drops
    COPPICE_SOCK/COPPICE_PANE at start). Without these explicit request
    fields the gate has no way back to THIS pane's coppice socket."""
    handler = type("H", (_CapturingReportHandler,), {"captured": []})
    sock_path = tmp_path / "g.sock"
    _serve(sock_path, handler)

    claimed_coppice_sock = str(tmp_path / "c.sock")
    result = _report(
        sock_path,
        "idle",
        {"COPPICE_PANE": "w1:p3", "COPPICE_SOCK": claimed_coppice_sock},
    )
    assert result.returncode == 0, result.stderr
    req, _payload = handler.captured[0]
    assert req["coppice_pane"] == "w1:p3"
    assert req["coppice_sock"] == claimed_coppice_sock


@_NODE
def test_report_state_carries_no_coppice_sock_field_with_only_the_pane_set(tmp_path):
    handler = type("H", (_CapturingReportHandler,), {"captured": []})
    sock_path = tmp_path / "g.sock"
    _serve(sock_path, handler)

    result = _report(sock_path, "idle", {"COPPICE_PANE": "w1:p3"})
    assert result.returncode == 0, result.stderr
    req, _payload = handler.captured[0]
...
@_NODE
def test_ask_gate_carries_the_callers_coppice_sock_and_pane_on_the_request(tmp_path):
    captured: list = []
    ...
    verdict = _ask(
        ...
        "read",
        {"path": "/x"},
        extra_env={"COPPICE_PANE": "w2:p5", "COPPICE_SOCK": claimed_coppice_sock},
    )
    assert verdict == {"allow": True, "reason": ""}
    req = captured[0]
    assert req["coppice_pane"] == "w2:p5"
    assert req["coppice_sock"] == claimed_coppice_sock
...
@_NODE
def test_ask_gate_carries_the_callers_coppice_data_dir_on_the_request(tmp_path):
    captured: list = []
    ...
    sock_path = tmp_path / "g.sock"
    _serve(sock_path, _CapturingHandler)
    _ask(sock_path, "read", {"path": "/x"}, extra_env={"COPPICE_DATA_DIR": "/srv/cdata"})

...
| Gate onboarding (`daisugi start` / `gate init`) | v0.38.0 | Working | Roadmap Stage 6. `starter_envelope` generates a reviewable tight default; `daisugi start` (v0.43.0, plan 5) is the one command — find the harness, install the hook (scoped to this directory), register the envelope, start the resident gate, open the view. Tutorial in `docs/tutorials/`. Time-to-first not yet measured on a clean machine. |
...
| Lexical matcher (`matcher_model: lexical`) | v0.44.0 | Working | ADR-0019. Zero model, zero download, numpy only. Fallback when the configured embedder's package is absent (warned once). Threshold 0.25, FPR-matched to MiniLM@0.55. |
| `coppice`, the Go floor daemon | the October 2026 fold | Working | One static binary owning panes, harness processes, and their merged state over a uid-checked unix socket. State comes from the gate first, screen detection second, never a guessed idle. A restart brings back layout and labels, never a running process. `coppice` with no arguments is the floor: the roster grouped by who needs you, a peek on one pane, and attach; `daisugi coppice floor` execs it. The Codex and sprig adapters have not run against a real binary yet; neither `coppice attach` nor the floor has a `--remote` form. |
| Phone client (`coppice web serve`) | the October 2026 fold | Experimental | The PWA coppice-server embeds: roster, pane grid, prompt, steer, allow, deny, and ntfy push on the transition to blocked. TLS from `tailscale cert`, a local CA, or explicit files. Web Push is planned, not built: `--web-push` exits 1 and names ntfy. Covered by Go and Node tests. A Playwright smoke pass drives a real Chromium browser over both TLS listeners and checks in five phone-sized screenshots. It runs behind `COPPICE_SMOKE=1` and does not run in CI. Not yet used on a phone for a week. |
| Voice bridge (`daisugi voice serve\|ptt\|arm\|disarm`) | the October 2026 fold | Experimental | `voice serve` answers GET /health, POST /transcribe, and POST /deliver, transcribing with faster-whisper on CPU by default. `voice ptt` is the laptop push-to-talk client. Delivered text previews in the pane until `voice arm PANE --for 30m` grants a time-boxed direct-send window; `voice disarm PANE` revokes it early. Parakeet is a second, opt-in engine needing sherpa-onnx and a downloaded model, about 460 MB, never fetched automatically. The optional cleanup pass runs one fixed-prompt local-model call and never changes what was said. The phone's record button is built and posts through `coppice web serve --voice-url`. Real microphone hardware, the phone's record button on a real phone, and Parakeet are all untested on this box. |

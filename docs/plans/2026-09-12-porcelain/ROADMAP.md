...
- Six keys in the roster: ctrl-t, shift-tab, ctrl-w, Space, Enter, Esc. A
  click on a tile gives that pane the keyboard, and every key then goes to
  the pane, Enter included. One stolen key gives the keyboard back:
  ctrl-space, which Claude Code leaves unbound. Twice sends it to the pane.
  Set `[keys] leave` in coppice.toml to change it. No prefix, no chords.
...
- **Plan 18b, live tiles.** Tiles take the keyboard on a click, ctrl-space
  gives it back, and the phone shows one pane above a dock of chips.
...
Plan 26 puts a name on every connection, a name on every allow and deny in
the journal, and `· alice looking` on the roster. A name is a label. It gives
no right, and the tiers from plan 20 hold for everyone. These wait:

- Shared tiles. Two people who type into one pane at once get no turn order
  and no mark of whose keys went in.
- Chat. People talk on a channel they already use. The floor carries no
  messages between them.
- Per-person envelopes. The gate checks one envelope per session, whoever
  answers. A name does not change what the gate allows.
- Roles beyond a name. Every operator may allow, named or not. There is no
  observer-only role, and no name may allow more than another.

...
Repo: this repository (git, branch master). Do not run git commands
that change state. Do not modify any file except the plan file you are told to create.

## Read, in this order
1. The superpowers writing-plans skill.
...
- `/tmp` may be RAM (some distributions mount it as tmpfs): scratch under the repo or
  `~/.cache`, never `/tmp`.

...
- The Parakeet-TDT-0.6B-v2 int8 archive is `sherpa-onnx-nemo-parakeet-tdt-0.6b-v2-int8.tar.bz2`
  ...
  auto-downloaded (some systems mount `/tmp` as RAM, and the model is an opt-in engine).
...
~600 MB and some systems mount `/tmp` as RAM."""
...
CUDA is opt-in-and-verified in pick_engine(), never auto-detected — a Pascal
GPU (sm_61) has a history of crashing torch-based inference under CUDA. The WER test

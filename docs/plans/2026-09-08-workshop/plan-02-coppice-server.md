...
- This box runs `go1.25.12` and Fedora ships it with `GOTOOLCHAIN=local` compiled in, so the
  toolchain will not switch on its own. `golang.org/toolchain` has `go1.26.8.linux-amd64`.
...
refuses to build against it from a 1.25 main module. This box ships go1.25.12 with
`GOTOOLCHAIN=local` compiled in, so it will not switch on its own.
...
Find the section of `/mnt/tera/working/programming/openDaisugi/AGENTS.md` that maps the

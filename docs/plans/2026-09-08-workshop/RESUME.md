...
| plan-06-phone.md | spec-06 (L) | opus | spec-02/03; fetch tailscale.com/kb/1153/enabling-https, docs.ntfy.sh/publish, github.com/mdp/qrterminal; visual-loop skill at /mnt/tera/working/programming/iss-skills/skills/visual-loop; localca fully specified (crypto/x509) |
| plan-07-voice.md | spec-07 (M) | sonnet | spec-03/06, gateway.py, gateway_journal.py, config.py, cli.py, pyproject extras; faster-whisper + sherpa-onnx APIs; GTX 1060 crashes torch → CPU default; WAV fixtures via espeak-ng if present else skip-with-reason |
...
- **The floor screen never lies and never dies.** The roster never attaches and never
  ...
  on the next resume; the attach screen pops itself at most once; `ctrl+a ctrl+a` sends a
  literal leader and only chords in `keys.json` are sent. A text-only backend refuses attach

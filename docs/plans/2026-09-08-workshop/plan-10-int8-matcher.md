...
real onnxruntime session, a real local journal — and reproduces the plan's printed
...
- **NOTE — the manifest and the derived threshold are real, not fabricated.** Re-verified
  ...
  tokenize→onnxruntime→mean-pool→L2-normalize pipeline against a real local journal
  (`~/.opendaisugi/journal/index.db`, the same corpus ADR-0019 used) —
  the FPR table, the `0.45` threshold, and the two labeled-pair scores in ADR-0021 all
  reproduce exactly. No correction needed here; noted so the BLOCKER above isn't mistaken
  for a sign the underlying data is suspect too.
...
- `_INT8_THRESHOLD`: derived by actually running `scripts/matcher_fpr.py` against a real local
  journal (`~/.opendaisugi/journal/index.db`) — the exact same method
...
(This is the actual output measured on a real local journal — 20,000 random distinct-task
pairs, seed 42, target FPR 2.56% — the same corpus and method
ADR-0018/0019 used, run on the `avx2` build the dev machine's CPU selects. If your journal differs, your
...
# scripts/matcher_fpr.py` against a real local journal (20,000 random
# distinct-task pairs, seed 42), closest to MiniLM@0.55's
...
5. **A threshold derived on the real journal, not guessed.** Running
   ...
   `~/.opendaisugi/journal/index.db` (20,000
   ...
   `_INT8_THRESHOLD = 0.45` (measured on the `avx2` build the dev machine's CPU
   selects; the three byte-identical builds would measure the same). On the same
...
- **GPU execution providers (CUDA, DirectML).** Out of scope. onnxruntime
  supports them, but a Pascal GPU (sm_61) is exactly the case ADR-0018

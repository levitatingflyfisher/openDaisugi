...
a few hundred distinct tasks that is tens of thousands of pairs, which is more data than the
sample and still one pass, so the numbers get tighter, never looser. Record that in the commit
message.

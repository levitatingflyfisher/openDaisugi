...
envelope field and the optional `opendaisugi[shell]` extra. Reachable from the CLI since
`daisugi gate init --allow-shell-decomposition` (and `gate quickstart`); before that the
field could only be set by hand-editing a registered envelope, so no gated session ever
had it on. Reachable from the bulk paths — `onboard`, `journal ingest`, `hook to-trace`,
`hook auto-tend`, `generate-envelope` — since the same flag was shared across them, with
the default persisted as `Config.shell_allow_decomposition` because `hook auto-tend` runs
from cron with no argv to carry a flag.

**Where the opt-in actually recovers work** (measured over the 250 largest real
transcripts — 1,582 episodes, 1,275 touching the shell — with the shell dimension
isolated): only the capture path, and only with both halves. `infer_envelope` now
decomposes to collect *every* head. Episodes verifying today: 353. With the flag alone:
355. With every head alone: 353. With both: 401 — so the fix recovers 48 of the 1,229
rejected episodes (22.3% → 25.3%) and each half alone recovers nothing. The gain is modest
because an episode passes only if every step passes, and a long session usually contains at
least one redirection. On the ingest path the envelope is generated from task text, so its
allowlist rarely names the plumbing heads real compound commands use (`echo`, `grep`,
`head`, `cd`, `sed` lead the corpus) and the opt-in recovers 2 episodes of 1,229. Corpus
shape: of 13,232 captured shell calls, 96.2% carry a metacharacter and 29.4% of those
decompose — the rest stay fail-closed on redirection (7,743), substitution (1,137), parse
errors (49), and wrappers or non-literal heads (34).

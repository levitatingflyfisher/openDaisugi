...
5. **A threshold derived on the real journal, not guessed.** The command is:
   ...
   It ran on 2026-09-08 against `~/.opendaisugi/journal/index.db`, 258
   distinct usable tasks, 20,000 random distinct-task pairs, seed 42, the
...
- **GPU execution providers, CUDA or DirectML.** Out of scope. onnxruntime
  supports them, but this box's Pascal GPU is the case ADR-0018 named as

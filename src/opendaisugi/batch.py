...
def classify_declaration(decl: BatchDeclaration) -> BatchClassification:
    ...
    non_batchable = [
        {"id": s.id, "type": s.type} for s in decl.program.steps if not is_batchable_type(s.type)
    ]
...
def prove_footprint(decl: BatchDeclaration, envelope: Envelope) -> FootprintProof:
    ...
    if under_declared:
        reasons.append(
            f"{len(under_declared)} write(s) outside the declared footprint F: {under_declared}"
        )

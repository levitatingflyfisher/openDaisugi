...
class Distiller:
    ...
    async def _distill_cluster(
        ...
        # Do-nothing salvage first (no LLM involved): a cluster whose plans
        ...
        try:
            ...
            divergent, salvage_params = plan_divergence(all_plans)
        ...
        if divergent and workspace is None:
            ...
            if salvage_check.ok and not salvage_timeout:
                try:
                    ...
                    salvage_sig = None
                _log.info(
                    "cluster salvaged: size=%d divergent_steps=%s typed_holes=%d",
                    len(cluster_traces),
                    divergent,
                    len(salvage_params),
                )
...
async def adapt_plan(
    ...
    *,
    ...
    try:
        # get_instructor_client can now raise LLMNotConfigured (preflight, v0.41)
        # before any network call — that is an LLM failure too, so it belongs
        # inside this try/except and falls back to the template like any other.
        client = get_instructor_client(model)

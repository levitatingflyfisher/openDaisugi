...
def hook_record_cmd(
    ...
    """
    if event not in ("pre_tool_use", "stop", "notification"):
        ...
        typer.echo("Error: --event must be one of pre_tool_use, stop, notification.", err=True)
        raise typer.Exit(code=1)
...
def setup_cmd(
    ...
    remote: str = typer.Option(
        ...
        "or the LAN. Example: --remote my-3090:11434.",
    ),
...
def coppice_spawn_cmd(
    ...
    if kind not in _VALID_KINDS:
        _fail(
            ...
            f"Try: daisugi coppice spawn --kind headless --cwd {cwd} -- claude",
        )
...
def install_cmd(
    ...
    harness: list[str] = typer.Option(
        None,
        "--harness",
        help="Install a loop harness's gate extension. Supported: pi. Independent of --runtime.",
    ),
    ...
    if harness:
        ...
        if do_uninstall:
            for h in harness:
                if dry_run:
                    target = home / ".pi" / "agent" / "extensions" / "daisugi-gate"
        ...
        if dry_run:
            for h in harness:
                target = home / ".pi" / "agent" / "extensions" / "daisugi-gate" / "index.ts"
                typer.echo(f"[{h}] would write {target}")
            return
        for h in harness:

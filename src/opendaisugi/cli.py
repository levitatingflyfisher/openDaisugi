...
def journal_ingest_cmd(
    ...
    if json_output:
        ...
        typer.echo(f"{verb} {summary.total} episodes from {episodes_file.name}"
...
def install_cmd(
    ...
    plans = {
        rt.name: rt.plan(home, selected_layers, enforce=enforce, base_url=base_url)
        for rt in runtimes
    }

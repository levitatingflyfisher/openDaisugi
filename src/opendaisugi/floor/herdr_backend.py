...
from opendaisugi.floor import Frame, PaneInfo, PaneRef, PaneStateEvent, TaskInfo, merge
...
class HerdrBackend:
    ...
    def spawn(
        ...
        *,
        ...
        it is ignored. `task` is refused: Herdr has no tasks.
        """
        if task is not None:
            raise self._no_tasks()
    ...
    def _no_tasks(self) -> NotImplementedError:
        return NotImplementedError(f"{self.name} has no tasks. Use the coppice backend.")

    def list_tasks(self) -> list[TaskInfo]:
        """No tasks here. The list is empty, never an error."""
        return []

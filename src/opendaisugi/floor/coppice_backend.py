...
class CoppiceBackend:
    ...
    def spawn(
        ...
        *,
        ...
        """Start a pane. With ``task`` the pane works for that task, and
        ``cwd`` may be None: a task with a worktree runs the pane there,
        and one without lends its own cwd. The server refuses a task it
        does not know, and a cwd that differs from the task's worktree."""
        fields: dict = {
            "cmd_argv": list(cmd),
            "env": dict(env),
            "label": label,
        ...
        if task is not None:
            fields["task"] = task
        result = self._call("pane.create", **fields)
        return PaneRef(backend=self.name, id=str(result["pane"]))

    @staticmethod
    def _task_from_row(row: dict) -> TaskInfo:
        return TaskInfo(
            ref=str(row["id"]),
            ...
            panes=tuple(str(p) for p in row.get("panes", [])),
        )

    def list_tasks(self) -> list[TaskInfo]:
        """Every task the server knows about, or an empty list. A host that
        is gone answers OSError at connect time, and a server that lacks the
        verb refuses it with a CoppiceError; both are an empty list here,
        never a raise."""
        try:
            result = self._call("task.list")
        except (OSError, CoppiceError):
            return []
        return [self._task_from_row(row) for row in result.get("tasks", [])]
    ...
    def create_task(
        ...
        *,
        ...
        """Record a task. With ``worktree`` the server adds a git worktree
        beside the repo that holds ``cwd``; a cwd outside a repo, a label
        git cannot use, or a repo on no branch is a CoppiceError."""
        fields: dict = {"label": label, "worktree": bool(worktree)}
        ...
        if model is not None:
            fields["model"] = model
        result = self._call("task.create", **fields)
        ref = str(result["task"])
    ...
    def close_task(self, task: str, *, keep_worktree: bool = False) -> None:
        """Close the task, its descendants, and every pane under them. With
        ``keep_worktree`` false the server refuses when any worktree in the
        task has uncommitted changes, and closes nothing."""
        self._call("task.close", task=task, keep_worktree=bool(keep_worktree))
    ...
    def find_ended(self, pane: PaneRef) -> PaneStateEvent | None:
        """The pane's own stored state, read from Recent (`pane.list`
        with `ended: true`), when it has already ended and coppice still
        has the record. None when the pane is live, was never known, or
        its record has already been forgotten or resumed.

        wait_for_state calls this, when it has it, so a wait that starts
        after the pane already ended - or that races its own end before
        the first poll - still reports done. Without it, wait_for_state's
        own absence heuristic needs to have seen the pane alive at least
        once first, which a pane already gone by the first poll never
        gets the chance to be. The event's own `detail` carries the real
        exit code, `exit=<n>`, for a pty pane - the same string
        `internal/server/panes.go`'s watchExit stamped on it server side;
        nothing here invents one.
        """
        try:
            result = self._call("pane.list", ended=True)
        except (OSError, CoppiceError):
            return None
        for row in result.get("panes", []):
            if row.get("id") == pane.id:
                return _state_from_pane_row(row)
        return None

...
def coppice_config_path(env: Mapping[str, str] | None = None) -> Path:
    """$XDG_CONFIG_HOME/coppice/coppice.toml, else ~/.config/coppice/coppice.toml."""
    env = os.environ if env is None else env
    base = env.get("XDG_CONFIG_HOME") or ""
    root = Path(base) if base else Path.home() / ".config"
    return root / "coppice" / "coppice.toml"


def harness_command(name: str, *, env: Mapping[str, str] | None = None) -> list[str] | None:
    """[command, *args] from [harness.<name>] in the coppice config, or None.

    A missing file, a file that does not parse, a missing table, or a table
    with no command is None. This never raises for a bad file: the caller
    says what to do.
    """
    try:
        data = tomllib.loads(coppice_config_path(env).read_text(encoding="utf-8"))
    except (OSError, tomllib.TOMLDecodeError, UnicodeDecodeError):
        return None
    tables = data.get("harness")
...
def prompt_pane(
    ...
    *,
    ...
    foreman, when true, sends the promotion line first, so the pane reads
    the foreman page before the text. It waits up to timeout_s for the pane
    to go idle, and raises PromotionNotSent, sending nothing, when it does
    not. With empty text it sends only that line.
    ...
    """
    if foreman:
        from opendaisugi.floor.promote import promote

        promote(backend, ref, timeout_s=timeout_s)
        if not text:
            return "typed"
    if kind is None:
        try:
...
def wait_for_state(
    ...
    *,
    ...
    A backend that can look up an ended record directly - find_ended, an
    optional method the same way list_proven is - answers the absent case
    with a real fact instead of an inference: a pane already gone before
    this wait's first poll, or one that ends between two polls, never had
    to be "seen alive" first the way the absence heuristic below needs.
    """
    list_proven = getattr(backend, "list_proven", None)
    find_ended = getattr(backend, "find_ended", None)
    ...
    while True:
        ...
        if info is None:
            if callable(find_ended) and until in ("done", "any"):
                ended = find_ended(ref)
                if ended is not None and (until == "any" or ended.state == until):
                    return ended

...
@dataclass(frozen=True)
class TaskInfo:
    """One task as the backend reports it. ref is the task id. worktree is
    the directory of its git worktree, or "". state is the worst state of
    its panes and children, or "" when nothing runs under it."""
...
class PaneBackend(Protocol):
    ...
    # ``task`` is the id of the task the pane works for. Only coppice has
    # tasks; herdr and tmux raise NotImplementedError when it is set. A
    # coppice task with a worktree runs the pane there, so a caller may
    # leave ``cwd`` unset on that backend.
    ...
    # Tasks sit above panes. list_tasks never raises and answers [] on a
    # backend with no tasks. create_task and close_task raise
    # NotImplementedError on herdr and tmux, naming the backend.

...
class AskActionsMixin:
    ...
    def _needs_strong_guard(self, row: SessionRow) -> bool:
        ...
        a = row.pending_ask or {}
        # The gate's tier comes first: only an ask the gate called undoable
        # may use the two-step. A missing tier is permanent.
        if a.get("tier") != "undoable":
            return True
    ...
    def action_arm_allow(self) -> None:
        ...
        if self._needs_strong_guard(row):
            ...
            self.app.set_status(
                f"HIGH-BLAST: this cannot be undone. Type the session id to allow: '{token}'"
                f" then press Enter to allow {tid}"
            )
        else:
            # Low-blast: the low-effort two-step. a arms, Enter confirms, any cursor
            # move disarms (on_data_table_row_highlighted).
            self._armed = tid
            self._cmd_mode = None
            self.app.set_status(
                f"allow {tid}? press Enter to confirm allow · move the cursor to cancel"
            )
    ...
    def action_confirm_allow(self) -> bool:
        ...
        if (
            ...
            # The writer checks the ask file again: the tier there decides,
            # not the row read at arm time.
            refusal = ask.check_allow(
    ...
    def handle_allow_token(self, mode: tuple, value: str) -> bool:
        ...
        if (
            ...
            return True
        refusal = ask.check_allow(
            self.app.data_dir / "gate", tool_use_id=tid, name=token, confirm=value
        )
        if refusal is not None:
            self.app.set_status(refusal)
            return True

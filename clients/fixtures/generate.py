...
HEADS = [
    ...
    (".venv/bin/sub/python", [".venv/bin/*"]),
    ("/abs/.venv/bin/python", [".venv/bin/*"]),
    ("/usr/bin/python", ["/usr/bin/*"]),
    ("usr/bin/python", ["/usr/bin/*"]),
    ("/usr/bin/python", ["/usr/*/python"]),
    ("python3", ["python[23]"]),
    ("python4", ["python[23]"]),
    ("a/b", ["*"]),
    ("ab", ["*"]),
...
def interp_case(cmd):
    ...
    return {
        "command": cmd,
        "payload": {"head": p.head, "opaque": p.opaque, "inner_commands": list(p.inner_commands)},
    }


def env(stakes):
    return Envelope(generated_by="fixture", task="t", stakes=stakes, permissions=Permission())


fixture = {
    "v": 1,
    "head_allowed": [{"head": h, "allowlist": a, "allowed": _head_allowed(h, a)} for h, a in HEADS],
    "path_match": [{"path": p, "globs": g, "matched": _path_matches_any(p, g)} for p, g in PATHS],
    "extract_head": [{"line": l, "head": _extract_shell_head(l.strip())} for l in LINES],
    "metachar": [{"command": c, "hit": bool(_SHELL_METACHAR_RE.search(c))} for c in METACHAR],
    "interpreter": [interp_case(c) for c in INTERP],
    "resolve_strict": [
        {"strict": s, "stakes": st, "effective": resolve_strict(s, env(st))}
        for s in (None, True, False)
        for st in ("low", "medium", "high", "physical")
    ],
}

...
def main() -> int:
    ...
    print(
        f"superset_heads (both True, ours is a supersequence — likely GLR-ambiguity artifact): {len(superset_heads)}"
    )
    print(
        f"other_mismatch (both True, genuinely different heads/reads/writes): {len(other_mismatch)}"
    )
    print()

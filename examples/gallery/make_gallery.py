...
def _mv(t):
    return ActionPlan(
        source="t", task="x", steps=[CartesianMoveStep(id="s", target_position=(t[0], t[1], 4.0))]
    )
...
def check_gates():
    ...
    checks = {
        ...
        "no_fly (obstacle)": (
            verify(_path([(4, 6), (15, 10.5), (25, 6)]), ob).ok,
            not verify(_path([(4, 6), (24, 6)]), ob).ok,
        ),
        "deconflict (aabb)": (
            aabb_disjoint(R((6, 6)), R((24, 6)), margin=3.5),
            not aabb_disjoint(R((13.5, 6)), R((16.5, 6)), margin=3.5),
        ),
        "delegation (subsumes)": (
            sub(E(((glo[0] + 1, 3, 0), (ghi[0] - 1, 9, 8)))),
            not sub(E(((glo[0] + 1, 3, 0), (ghi[0] + 12, 9, 8)))),
        ),
        "formation (per-lane)": (
            verify(_mv((10, 6)), E(((7.5, 2, 0), (14, 10, 8)))).ok,
            not verify(_mv((16, 6)), E(((7.5, 2, 0), (14, 10, 8)))).ok,
        ),
        "human_keepout (dyn obs)": (
            verify(_mv((10, 5)), E(box)).ok,
            not verify(
                _mv((16, 6)),
                Envelope(
                    generated_by="t",
                    ...
                    permissions=Permission(
                        workspace_bounds=box, obstacles=[((14, 4, 0), (18, 8, 8))]
                    ),
                    invariants=[
                        Invariant(type="end_effector_in_workspace", description="in"),
                        Invariant(type="no_obstacle_penetration", description="avoid"),
        ...
        "intercept (geofence)": (
            verify(_mv((20, 6)), E(((0, 0, 0), (28, 12, 8)))).ok,
        ...
        "reassignment (swarm)": (
            verify_swarm_tasking(
                mission,
                {
                    "w": E(
                        (
                            grant.permissions.workspace_bounds[0],
                            fleet["m"].permissions.workspace_bounds[1],
                        )
                    ),
                    "e": fleet["e"],
                },
            ...
            not verify_swarm_tasking(
                mission,
                {
                    ...
                    "e": E(
                        (
                            fleet["m"].permissions.workspace_bounds[0],
                            fleet["e"].permissions.workspace_bounds[1],
                        )
                    ),
                },
                margin=0.5,
        ...
        "cross_swarm (aabb)": (
            aabb_disjoint(((0, 0, 0), (15, 12, 8)), ((16, 0, 0), (30, 12, 8)), margin=0.5),
    ...
    slalom_env = Envelope(
        ...
        stakes="physical",
        permissions=Permission(
            workspace_bounds=box, obstacles=[((8, 4, 0), (11, 8, 8)), ((19, 4, 0), (22, 8, 8))]
        ),
        invariants=[
            Invariant(type="end_effector_in_workspace", description="in"),
    ...
    checks.update(
        {
            "swarm_of_swarms (nested)": (
                envelope_subsumes(mission, adr["a1"]).holds,
                not envelope_subsumes(squads["br"], E(((salx, 3, 0), (sblx + 4, 9, 8)))).holds,
            ),
            "slalom (multi-obstacle)": (
                verify(_path([(3, 6), (6, 11), (24, 11), (27, 6)]), slalom_env).ok,
                not verify(_path([(3, 6), (27, 6)]), slalom_env).ok,
            ),
            "handoff (lateral)": (
                envelope_subsumes(b_env, E(((13, 4, 0), (23, 9, 8)))).holds,
                not envelope_subsumes(b_env, E(((13, 4, 0), (30, 9, 8)))).holds,
            ),
            "leash (tether)": (
                verify(_mv((10, 6)), E(((4, 2, 0), (12, 10, 8)))).ok,
            ...
            "corridor (merge)": (
                aabb_disjoint(R((4, 6)), R((26, 6)), margin=3.0),
                not aabb_disjoint(R((14, 6)), R((16, 6)), margin=3.0),
            ),
            "return_to_base (invar)": (
                verify(
                    ActionPlan(
                        source="t",
                        task="x",
                        steps=[*patrol, S._ReturnToBase(id="c", depends_on=["a"])],
                    ),
                    rtb_env,
                ).ok,

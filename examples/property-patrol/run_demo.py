...
def _coordinator(bounds, *, velocity=3.0):
    ...
    return Envelope(
        generated_by="patrol-coordinator",
...
def _clamp_into(sector_bounds, target):
    ...
    return (
        min(max(target[0], lo_x + DRONE_RADIUS), hi_x - DRONE_RADIUS),
...
def main():
    ...
    sectors = partition_and_assign(
        coordinator, fleet, axis=0, margin=SECTOR_MARGIN
    )  # split along x
    ...
    print(
        f"   static sector gap = {SECTOR_MARGIN}m; live safety bubble = {SAFETY_BUBBLE}m (stricter, in-motion)"
    )
    ...
    for tick in range(1, 7):
        ...
        for did in fleet:
            ...
            if r.ok:
                ...
                note = (
                    f"proposes {tuple(round(v, 1) for v in proposed)} → ✗ REJECTED "
                    f"({reason})\n              └─ Simplex fallback → "
            ...
            conflict = next(
                (
                    ...
                    for o in fleet
                    if o != did
                    and not aabb_disjoint(
                        _occupies(target), _occupies(committed[o]), margin=SAFETY_BUBBLE
                    )
                ),
                None,
            )

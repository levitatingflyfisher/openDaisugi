...
|---|---|
| `GOTOOLCHAIN` | `auto` |
| `PKG_CONFIG` | `/home/inspire/.local/bin/coppice-pkg-config` |
...
| `tailscale cert` flags | `--cert-file value` and `--key-file value` | `tailscale cert --help` on this box. Verified: yes. `tailscale` is present at `/usr/bin/tailscale`. |

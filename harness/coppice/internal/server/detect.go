...
// GateQuiet is the hold state.Merge's rule 5 gives an incoming manifest
...
// GateQuiet - scanOnce's own skip and Merge's rule 5 both measure the
...
func (s *Server) tickSkipReason(paneID string, now float64) string {
	...
	// Merge rule 5: an ordinary (non-ask) hold on a higher-rank source

...
func TestAForkStaysPendingUntilItsAdapterReportsANewSessionID(t *testing.T) {
	...
	// The saved layout agrees. The tree in memory changes before the file
	// is saved, so the check waits for the file.
	...
	for {
		...
		saved, _ := tree.Pane(newID)
		if !saved.ForkPending && saved.HarnessSessionID == "sess-child" {
			break
		}
...
// An operator pane.close removes the record at once, so a fork of it
// afterward is no_such_pane, the same as forking an id that never existed -
// there is no longer a distinct "closed but still on record" case to tell
// apart from "never existed" here.
func TestForkOfAClosedOrUnknownPaneIsRefusedByCode(t *testing.T) {
	...
	got := roundTrip(t, s,
		...
		`{"id":"4","cmd":"pane.fork"}`)
	if got[1].OK || got[1].Error.Code != proto.ErrNoSuchPane {
		t.Fatalf("got %+v, want no_such_pane", got[1])
	}

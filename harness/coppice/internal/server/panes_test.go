...
// An operator pane.close removes the record at once: the id no longer
// names anything, so a later verb against it is no_such_pane, not
// pane_closed - there is nothing left even to call closed.
func TestClosedPaneRejectsInputWithNoSuchPane(t *testing.T) {
	s := newPaneServer(t)
	cwd := t.TempDir()
	got := roundTrip(t, s,
...
func TestProcessExitMarksThePaneDoneFromTheProcessSource(t *testing.T) {
	...
	if p["ended_at"] == nil {
		t.Fatalf("ended row has no ended_at: %v", p)
	}
}
...
// After an operator pane.close, pane.read and pane.wait_output must refuse
// AT ONCE - no polling to a timeout, because there is nothing left to poll:
// close already removed the record. The code is no_such_pane now (an
// operator close removes the record, it does not just mark it closed), but
// the "at once, no polling" guarantee this test exists for is unchanged.
func TestReadAfterCloseIsNoSuchPaneAtOnce(t *testing.T) {
	s := newPaneServer(t)
	cwd := t.TempDir()
	got := roundTrip(t, s,
		`{"id":"1","cmd":"pane.create","cwd":"`+cwd+`","cmd_argv":["sh","-c","sleep 2"],"kind":"pty"}`,
	...
	if got2[0].OK || got2[0].Error.Code != proto.ErrNoSuchPane {
		t.Fatalf("pane.read after close = %+v, want no_such_pane", got2[0])
	}
	if got2[1].OK || got2[1].Error.Code != proto.ErrNoSuchPane {
		t.Fatalf("pane.wait_output after close = %+v, want no_such_pane", got2[1])
	}
...
// left for this test binary to fail against: pi and opencode register in
// their own packages, internal/adapters/pi and internal/adapters/opencode,
// which this test binary never blank-imports -
...
//
// claude, codex, pi, opencode and sprig are real packages, but this test
// binary never blank-imports any of the five, so none of them is registered here at all -

...
func TestRestoreReportsWhatItCouldAndCouldNotDo(t *testing.T) {
	...
	roundTrip(t, s1,
		...
		// A pane already closed before the restart must be counted on its
		// own, not folded into MarkedDone/MarkedUnknown alongside the panes
		// that were actually open when the daemon went away.
		`{"id":"2","cmd":"pane.create","cwd":"`+cwd+`","cmd_argv":["sh","-c","sleep 5"],"kind":"pty"}`,
		`{"id":"3","cmd":"pane.close","pane":"w1:p2"}`)
	s1.saveLayout()
	_ = s1.Close()

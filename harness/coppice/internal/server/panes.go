...
// closePane is updatePane's counterpart for a pane ending on its own: a
// process exit (watchExit) or a headless adapter's own end of session
// (pumpAdapter). It marks the record ended (Closed, EndedAt, ExitCode) and
// keeps it, so it can list under `ended: true` in Recent. It is NOT how an
// operator close works - see closeRecord and dropRecord, just below, for
// that path, which removes the record instead.
//
// It errors, without touching anything else, when id no longer names a
// record: an operator close (or a forget, resume, or the seven-day sweep)
// can remove a record while this pane's own exit goroutine is still on its
// way to report it. Every caller of closePane ignores that error on
// purpose - there is nothing left to close.
func (s *Server) closePane(id string, exit *int) error {
	if err := s.tree.ClosePane(id, exit, s.clock().Unix()); err != nil {
		return err
	}
	s.forgetChildren(id)
	s.forgetAsks(id)
	s.tree.DropForeman(id)
	s.refreshLiveInfo(id)
	s.saveLayout()
	return nil
}

// dropRecord is the one path that erases a pane's record for good: an
// operator pane.close, pane.forget, a successful pane.resume, and the
// seven-day sweep all end here. It frees the grid if one is still around -
// an ended pty or headless pane keeps its grid readable until this runs -
// stops the process tick, forgets the pane's stored state, asks and
// children, clears it as any task's foreman, and removes the tree record
// outright, never leaving a pane_closed zombie behind.
//
// removeLive runs before RemovePane: it stops the pty's own tick goroutine,
// so nothing can post a fresh fact for this id between the record leaving
// the tree and this call returning. Everything here is idempotent against a
// pane this call has already handled, or one with no LivePane left at all
// (an operator-closed pane already torn down by closeRecord), so a caller
...
var namedKeys = map[string]string{
	...
	"ctrl+space": "\x00", "shift+tab": "\x1b[Z",
...
// Every ctrl letter is a named key: ctrl+a is 0x01 through ctrl+z at 0x1a.
func init() {
	for c := 'a'; c <= 'z'; c++ {
		namedKeys["ctrl+"+string(c)] = string(rune(c - 'a' + 1))
	}
}
...
func (s *Server) RegisterPaneCommands() {
	...
	_ = s.Handle("pane.trust", s.handleTrust)
	...
	_ = s.Handle("pane.rename", s.handlePaneRename)
...
// harnessArgv builds the command line for a pty pane that names a harness
// and gives no cmd_argv: the [harness.<name>] command, then its args, then
// extra. ok is false when the config file is missing, does not parse, has
// no such table, or the table has no command.
func harnessArgv(name string, extra []string) ([]string, bool) {
	...
	argv := append([]string{h.Command}, h.Args...)
	return append(argv, extra...), true
}

func (s *Server) handlePaneCreate(c *Client, r *proto.Request) proto.Response {
	...
	cwd, _ := r.Str("cwd")
	// A pane born into a task runs where the task runs: in its worktree
	// when it has one, else in the task's cwd unless the request names its
	// own. A cwd that differs from the worktree is refused, never moved.
	taskID, _ := r.Str("task")
	if taskID != "" {
		tk, ok := s.tree.Task(taskID)
		if !ok {
			return noTask(r.ID, taskID)
		}
		switch {
		case tk.Worktree != "" && cwd != "" && cwd != tk.Worktree:
			return proto.ErrResp(r.ID, proto.ErrBadRequest,
				fmt.Sprintf("a pane in task %s runs in its worktree %s. Drop cwd.", tk.ID, tk.Worktree))
		case tk.Worktree != "":
			cwd = tk.Worktree
		case cwd == "":
			cwd = tk.Cwd
	...
	if harness != "" && !validHarness(harness) {
		return proto.ErrResp(r.ID, proto.ErrBadRequest, fmt.Sprintf(
			"harness must be a name of at most %d letters, digits, '.', '_' or '-', as in coppice.toml.", harnessMax))
	}

	// near names a pane whose own cwd stands in for one this request did
	// not give. It only ever matters for the fallback just below, but a
	// name that answers to nothing is refused outright rather than quietly
	// ignored - the caller meant to land beside a real pane.
	near, hasNear := r.Str("near")
	var nearCwd string
	if hasNear && near != "" {
		nearRec, ok := s.tree.Pane(near)
		if !ok {
			return proto.ErrResp(r.ID, proto.ErrNoSuchPane,
				fmt.Sprintf("no pane %q to start near. Run: coppice pane list", near))
		}
		nearCwd = nearRec.Cwd
	}

	// A one-click new agent: nothing about where or what to run was given.
	// This is the only case a missing cwd or harness gets a guess instead
	// of a refusal - a request that gives a cwd, an argv, or a harness of
	// its own is answered exactly as it asks, unchanged.
	autoDefault := cwd == "" && len(argv) == 0 && kind == layout.KindPTY
	if autoDefault {
		if resolved := s.defaultCwd(nearCwd); resolved != "" {
			cwd = resolved
		}
		if harness == "" {
			if name, ok := defaultHarnessName(); ok {
				harness = name
			}
		}
	}
	...
	// extraArgs is only ever set by the harness-table branch just below.
	// It rides on the record so pane.resume's pty path can put it back
	// after a harness's own resume_args, instead of losing it.
	...
	if kind == layout.KindPTY && len(argv) == 0 && harness != "" {
		// The config file is read on every such request, so an edit
		// takes effect on the next pane without a restart. A missing
		// file, a file that does not parse, or a table with no command
		// is the same refusal: the file has no command for this name.
		extra, _ := r.StrSlice("args")
		resolved, ok := harnessArgv(harness, extra)
		if !ok {
			return proto.ErrResp(r.ID, proto.ErrBadRequest,
				fmt.Sprintf("no harness named %q in %s. Run coppice once to write it, "+
					"or add [harness.%s] with command = %q.", harness, config.Path(), harness, harness))
		}
		argv = resolved
		extraArgs = extra
	}
	if kind == layout.KindPTY && len(argv) == 0 {
		if autoDefault && harness == "" {
			return proto.ErrResp(r.ID, proto.ErrBadRequest,
				fmt.Sprintf("no default harness in %s. Run coppice open HARNESS once to set one.", config.Path()))
		}
		return proto.ErrResp(r.ID, proto.ErrBadRequest,
			"a pty pane needs cmd_argv. Put the command after -- on the command line.")
	}

	wsID, tabID, err := s.placeFor(r, cwd)
	if err != nil {
		return proto.ErrResp(r.ID, proto.ErrInternal, err.Error())
	}
	...
	// A request that names no label of its own gets one made from the
	// harness and the directory's own name - a pane with no harness (a raw
	// cmd_argv) stays unlabelled, same as before. The lock spans the whole
	// pick-then-create, so two requests racing for the same harness and
	// directory can never both land on the same label: whichever wins the
	// lock is the one the other's own scan then sees as taken.
	needsLabel := label == "" && harness != "" && cwd != ""
	ro := roleOrOperator(c)
	if ro.noAllow && label != "" && s.liveLabelTaken(label) {
		return proto.ErrResp(r.ID, proto.ErrUnauthorized, LabelTakenRefusal)
	}
	if needsLabel {
		s.labelMu.Lock()
		label = s.defaultLabel(harness, cwd)
	}
	// The final label is checked, whatever made it: a pane never ends up
	// named foreman.
	if ro.noAllow && isForemanLabel(label) {
		if needsLabel {
			s.labelMu.Unlock()
		}
		return proto.ErrResp(r.ID, proto.ErrUnauthorized, ForemanLabelRefusal)
	}
	rec, err := s.tree.CreatePane(wsID, tabID, layout.Pane{
		Label: label, Cwd: cwd, Argv: argv, Env: env, Kind: kind,
		Harness: harness, Cols: cols, Rows: rows, TaskID: taskID,
		ExtraArgs: extraArgs,
	})
	if needsLabel {
		s.labelMu.Unlock()
...
func (s *Server) handlePaneFork(_ *Client, r *proto.Request) proto.Response {
	...
	label = cleanLabel(label)
	if label == "" {
		label = cleanLabel(info.Label + " fork")
	}
...
// handlePaneRename sets a pane's label in place, live or ended: the window
// and the row title, and the name a later pane.forget or pane.resume shows
// back. An empty label is refused rather than accepted as "clear it" - the
// operator asked to rename, not to blank the name out.
func (s *Server) handlePaneRename(c *Client, r *proto.Request) proto.Response {
	id, ok := r.Str("pane")
	if !ok || id == "" {
		return proto.ErrResp(r.ID, proto.ErrBadRequest,
	...
	cur, ok := s.tree.Pane(id)
	if !ok {
		return proto.ErrResp(r.ID, proto.ErrNoSuchPane,
			fmt.Sprintf("no pane %q. Run: coppice pane list", id))
	}
	if ro := roleOrOperator(c); ro.noAllow {
		if isForemanLabel(label) {
			return proto.ErrResp(r.ID, proto.ErrUnauthorized, ForemanLabelRefusal)
		}
		if label != cur.Label && s.liveLabelTaken(label) {
			return proto.ErrResp(r.ID, proto.ErrUnauthorized, LabelTakenRefusal)
		}
	}
	if err := s.updatePane(id, func(p *layout.Pane) { p.Label = label }); err != nil {
		return proto.ErrResp(r.ID, proto.ErrNoSuchPane, err.Error())
	}
	return proto.OKResp(r.ID, map[string]any{"pane": id, "label": label})
}
...
func (s *Server) startPane(rec layout.Pane) error {
	// A directory that is gone is named here. exec reports it as a missing
	// command, which sends the operator looking for a binary that is there.
	if rec.Cwd != "" {
		if fi, err := os.Stat(rec.Cwd); err != nil || !fi.IsDir() {
			return fmt.Errorf("the directory %s does not exist. Pick another directory.", rec.Cwd)
		}
	}
	...
	case layout.KindPTY:
		...
		p, err := pane.StartPTY(pane.SpawnOpts{
			...
			Sock: s.cfg.SocketPath, PaneID: rec.ID, DataDir: s.cfg.DataDir,
		}, g)
		if err != nil {
			g.Close()
			return err
...
// clock is the time the process source and the merge's hold window read:
...
func (s *Server) processTick(paneID string, lp *LivePane) {
	...
	if last, ok := lp.Grid.LastWrite(); ok {
		if s.clock().Sub(last) < processQuiet {
...
func (s *Server) pumpAdapter(paneID, harness string, p pane.Proc, g *pane.Grid) {
	...
	for ev := range p.Events() {
		if ev.Kind == pane.EvChild {
			s.applyChildEvent(paneID, ev)
			continue
		}
		write(ev)
		...
		if st == proto.StateBlocked {
			e.Ask = ev.Ask
			// A harness that answers its own asks marks them, so a client
			// never offers an Allow the gate cannot take.
			if _, owns := p.(pane.Answerer); owns && ev.Ask != nil {
				a := *ev.Ask
				a.Holder = proto.HolderHarness
				e.Ask = &a
			}
		}
	...
	// See watchExit's matching comment: an operator close can remove this
	// pane's record between closePane above finding nothing to close and
	// this line. The done event still had to go out for any concurrent
	// waiter; this drops it from the store once nothing is left to read it.
	if _, ok := s.tree.Pane(paneID); !ok {
		s.states.Forget(paneID)
	}
}
...
func (s *Server) watchExit(paneID string, p *pane.PTY) {
	...
	// An operator can close this pane between watchExit starting to tear
	// down its process and this line: closePane above then found no
	// record and did nothing, but ApplyState just posted done anyway (any
	// concurrent agent.wait/pane.wait_output on this id still needs to see
	// it). With no record left to read it, that state would sit in the
	// store forever; this is the one place that notices and drops it.
	if _, ok := s.tree.Pane(paneID); !ok {
		s.states.Forget(paneID)
	}
}
...
// handlePaneList lists live records, or with `ended: true`, ended ones
// only: the two are kept apart so a pane an operator is done with does not
// keep cluttering the working set, and one that ended on its own is not
// lost either. Ended rows come newest first and always carry `ended_at` and
// `exit_code`. quiet_for is seconds since the grid last took a byte, or
// since the entry started when it never has, and rides on every row that
// still has a grid - an ended pane keeps its grid, so it can too, until
// pane.forget or pane.resume frees it. With scope, only the panes of that
// task and its descendants are listed. path is the labels from the root to
// the scope.
func (s *Server) handlePaneList(_ *Client, r *proto.Request) proto.Response {
	...
	for _, p := range matched {
		out = append(out, s.paneListRow(p))
	}
	return proto.OKResp(r.ID, map[string]any{"panes": out, "path": path})
}

// paneListRow builds one pane.list row. A live pty pane always has a stored
// event, because startPane applies the first process fact before
// pane.create answers, so the unknown default below is reached only by a
// record with no live entry and no stored event.
func (s *Server) paneListRow(p layout.Pane) map[string]any {
	row := map[string]any{
	...
	if kids := s.childrenOf(p.ID); len(kids) > 0 && !p.Closed {
		row["children"] = kids
	}
	if names := s.looking(p.ID); len(names) > 0 {
		row["looking"] = names
	}
	...
	if eff, ok := s.effectiveState(p.ID); ok {
		...
		if h := s.heldFor(p.ID); h != nil && eff.State == proto.StateBlocked {
			row["held"] = h
		}
	} else if p.Closed {
		// No stored event at all, yet the record itself says closed: a
		// pane that already ended before this server's own restart, the
		// one case nothing else here ever populates the state store for.
		// See endedFallbackState.
		state, source, detail, ts := endedFallbackState(p)
		row["state"] = state
		row["source"] = source
		row["detail"] = detail
		row["ts"] = ts
...
// effectiveState is the state a reader sees for one pane now. It is the one
// lookup behind pane.list and task.list, so the two never disagree.
// EffectiveState is the read-time half of Merge's own expiry rule. Current
// alone hands back a blocked event forever, even long after its ask's
// deadline passed. EffectiveState turns that into working for a reader with
// no fresh event to merge against.
func (s *Server) effectiveState(id string) (*proto.PaneStateEvent, bool) {
...
func (s *Server) handleSendText(c *Client, r *proto.Request) proto.Response {
	...
	if id, _ := r.Str("pane"); isPromptRequest(r) && s.gated(id) {
		return s.promptReply(c, r, id, text)
	}
	...
	if id, _ := r.Str("pane"); id != "" {
		s.markInput(id, endsLine(text))
	}
...
func (s *Server) handleSendKeys(c *Client, r *proto.Request) proto.Response {
	...
	if id, _ := r.Str("pane"); id != "" {
		s.markInput(id, endsLine(out.String()))
	}
...
func (s *Server) handleRun(c *Client, r *proto.Request) proto.Response {
	...
	// On a harness pane under the ready-prompt rules, the line is a
	// prompt: refused on a question, queued while the pane starts or
	// works. See ready.go.
	if id := paneOf(r); s.gated(id) {
		return s.promptReply(c, r, id, line)
	}
	if id, _ := r.Str("pane"); id != "" {
		s.markInput(id, true)
	}
...
// closeRecord stops the pane's process, if it has one, and removes its
// record at once. pane.close and task.close both end here. An operator
// close never lands in Recent - that is the line between a pane the
// operator ends and one that ends on its own - so this stops the process
// and hands off to dropRecord instead of marking the record ended the way
// closePane does.
func (s *Server) closeRecord(rec layout.Pane) {
...
// placeFor is the workspace and tab a new pane goes in: the ones the
// request names, else the current ones. On a fresh server there are none,
// and the first create makes them. The lock makes several first creates at
// once share that one workspace, and a workspace whose tab is not made yet
// counts as none, so no create sees it half made.
func (s *Server) placeFor(r *proto.Request, cwd string) (string, string, error) {
	wsName, wsGiven := r.Str("workspace")
	tabName, tabGiven := r.Str("tab")
	if wsGiven && tabGiven {
		return wsName, tabName, nil
	}
	s.placeMu.Lock()
	...
	if wsID == "" || (tabID == "" && !wsGiven) {
		ws := s.tree.CreateWorkspace("", cwd)
		tab, err := s.tree.CreateTab(ws.ID, "main")
		if err != nil {
			return "", "", err
		}
		return ws.ID, tab.ID, nil
	}
	return wsID, tabID, nil
}

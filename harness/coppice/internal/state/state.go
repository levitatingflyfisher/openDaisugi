...
//     through to rules 5 and 6 below; it does NOT synthesize a "working" event
...
//  5. A manifest blocked read holds against a process idle: a question
//     that waits for a key goes quiet, and quiet is not an answer. Every
//     other process fact and every new manifest read applies.
//  6. Ordinary precedence: the 2 s window holds back cur only when the
...
//   - Rule 6, the hold-window clock: Python's merge() gates the window on
...
func Merge(cur *proto.PaneStateEvent, curReceived float64,
	...
	// waits for a key draws nothing more, so the process goes quiet, and
	// that quiet is not an answer. New output from the process, or a new
	// screen read, still applies.
	if cur.State == proto.StateBlocked && cur.Source == proto.SrcManifest &&
		in.Source == proto.SrcProcess && in.State == proto.StateIdle {
		return *cur
	}

...
// A screen that reads blocked stays blocked when the process only went
// quiet: a question that waits for a key draws nothing more. Any other
// process fact, and any later screen read, still applies.
func TestAScreenBlockedOutranksAProcessIdle(t *testing.T) {
	cur := ev(proto.StateBlocked, proto.SrcManifest, 100)
	cur.Ask = nil
	got := Merge(&cur, 100, ev(proto.StateIdle, proto.SrcProcess, 110), 110)
	if got.State != proto.StateBlocked || got.Source != proto.SrcManifest {
		t.Fatalf("a quiet process cleared a screen blocked: %s/%s", got.State, got.Source)
	}
	if got := Merge(&cur, 100, ev(proto.StateWorking, proto.SrcProcess, 110), 110); got.State != proto.StateWorking {
		t.Fatalf("new output did not clear the blocked read: %s", got.State)
	}
	if got := Merge(&cur, 100, ev(proto.StateIdle, proto.SrcManifest, 110), 110); got.State != proto.StateIdle {
		t.Fatalf("a new screen read did not apply: %s", got.State)
	}
}

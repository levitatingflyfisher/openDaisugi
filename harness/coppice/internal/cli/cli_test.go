...
func TestAnUnknownVerbExitsOneAndListsTheVerbs(t *testing.T) {
	// No config file and one fake harness on PATH: the word is neither a
	// verb nor a harness, so it is an unknown command.
...
func TestPaneRenameOverTheSocket(t *testing.T) {
	sock, dir := liveServer(t)
	cwd := t.TempDir()
	runCLI(t, sock, dir, "pane", "create", "--cwd", cwd, "--label", "one",
		"--", "sh", "-c", "sleep 300")
	code, out, errb := runCLI(t, sock, dir, "pane", "rename", "w1:p1", "new", "name")
	if code != 0 {
		t.Fatalf("pane rename exit %d: %s %s", code, out, errb)
	}
	code, out, _ = runCLI(t, sock, dir, "pane", "list", "--json")
	if code != 0 {
		t.Fatalf("pane list --json exit %d", code)
	...
	if row["label"] != "new name" {
		t.Fatalf("label = %v, want %q", row["label"], "new name")
	}
}
...
// pane list shows live panes only; a pane that ended on its own moves
// to Recent, `pane list --ended`. The table printer had no sign a pane's
// own row was ended there either - an ended pane and a live one printed
// identically. The state column is not a substitute: a pane that never
// started at all also shows closed with no state to say so.
func TestPaneListEndedTableMarksAnEndedRow(t *testing.T) {
	sock, dir := liveServer(t)
	cwd := t.TempDir()
	runCLI(t, sock, dir, "pane", "create", "--cwd", cwd, "--", "sh", "-c", "sleep 300")
	runCLI(t, sock, dir, "pane", "create", "--cwd", cwd, "--", "sh", "-c", "exit 0")
	code, out, errb := runCLI(t, sock, dir, "pane", "wait-output", "w1:p2", "--state", "done", "--timeout", "5000")
	if code != 0 {
		t.Fatalf("pane wait-output exit %d: %s %s", code, out, errb)
	}
	...
	if strings.Contains(out, "w1:p2") {
		t.Fatalf("pane list (live only) still shows the ended pane: %q", out)
	}
	if !strings.Contains(out, "w1:p1") {
		t.Fatalf("pane list dropped the live pane too: %q", out)
	}

	code, out, errb = runCLI(t, sock, dir, "pane", "list", "--ended")
	if code != 0 {
		t.Fatalf("pane list --ended exit %d: %s %s", code, out, errb)
	}
	...
	for _, l := range lines {
		...
		if strings.HasPrefix(l, "w1:p1 ") {
			t.Fatalf("pane list --ended shows the live pane: %q", l)
		}
	}
	if !strings.Contains(endedLine, "closed") {
		t.Fatalf("ended pane's row %q does not mark it closed", endedLine)
	}
}

// coppice pane forget PANE removes an ended record's CLI path.
func TestPaneForgetRemovesAnEndedPaneFromTheCLI(t *testing.T) {
	sock, dir := liveServer(t)
	cwd := t.TempDir()
	runCLI(t, sock, dir, "pane", "create", "--cwd", cwd, "--", "sh", "-c", "exit 0")
	code, out, errb := runCLI(t, sock, dir, "pane", "wait-output", "w1:p1", "--state", "done", "--timeout", "5000")
	if code != 0 {
		t.Fatalf("pane wait-output exit %d: %s %s", code, out, errb)
	}
	code, out, errb = runCLI(t, sock, dir, "pane", "forget", "w1:p1")
	if code != 0 {
		t.Fatalf("pane forget exit %d: %s %s", code, out, errb)
	}
	if !strings.Contains(out, "forgot") {
		t.Fatalf("pane forget printed %q, want a sign it happened", out)
	}
	code, out, errb = runCLI(t, sock, dir, "pane", "list", "--ended")
	if code != 0 {
		t.Fatalf("pane list --ended exit %d: %s %s", code, out, errb)
	...
	if strings.Contains(out, "w1:p1") {
		t.Fatalf("pane forget did not remove the record: %q", out)
	}
}

// coppice pane forget --ended removes every ended record at once.
func TestPaneForgetEndedRemovesEveryEndedRecordFromTheCLI(t *testing.T) {
	sock, dir := liveServer(t)
	cwd := t.TempDir()
	runCLI(t, sock, dir, "pane", "create", "--cwd", cwd, "--", "sh", "-c", "exit 0")
	runCLI(t, sock, dir, "pane", "create", "--cwd", cwd, "--", "sh", "-c", "exit 1")
	for _, id := range []string{"w1:p1", "w1:p2"} {
		code, out, errb := runCLI(t, sock, dir, "pane", "wait-output", id, "--state", "done", "--timeout", "5000")
		if code != 0 {
			t.Fatalf("pane wait-output %s exit %d: %s %s", id, code, out, errb)
		}
	}
	code, out, errb := runCLI(t, sock, dir, "pane", "forget", "--ended")
	if code != 0 {
		t.Fatalf("pane forget --ended exit %d: %s %s", code, out, errb)
	}
	if !strings.Contains(out, "forgot 2") {
		t.Fatalf("pane forget --ended printed %q, want it to say it forgot 2", out)
	}
	code, out, errb = runCLI(t, sock, dir, "pane", "list", "--ended")
	if code != 0 {
		t.Fatalf("pane list --ended exit %d: %s %s", code, out, errb)
	...
	if strings.TrimSpace(out) != "" {
		t.Fatalf("pane list --ended after forget --ended = %q, want empty", out)
	}
}

// coppice pane resume PANE starts a fresh pane from an ended one when the
// harness gives it nothing to resume, and removes the ended record.
func TestPaneResumeStartsFreshFromTheCLI(t *testing.T) {
	sock, dir := liveServer(t)
	cwd := t.TempDir()
	runCLI(t, sock, dir, "pane", "create", "--cwd", cwd, "--", "sh", "-c", "exit 0")
	code, out, errb := runCLI(t, sock, dir, "pane", "wait-output", "w1:p1", "--state", "done", "--timeout", "5000")
	...
	code, out, errb = runCLI(t, sock, dir, "pane", "resume", "w1:p1")
	if code != 0 {
		t.Fatalf("pane resume exit %d: %s %s", code, out, errb)
	}
	if !strings.Contains(out, "resumed false") {
		t.Fatalf("pane resume printed %q, want it to say resumed false", out)
	}
	code, out, errb = runCLI(t, sock, dir, "pane", "list", "--ended")
	if code != 0 {
		t.Fatalf("pane list --ended exit %d: %s %s", code, out, errb)
	...
	if strings.Contains(out, "w1:p1") {
		t.Fatalf("pane resume did not remove the old ended record: %q", out)
	}
}
...
// task create, task list, and task list --tree over the socket. The tree
// is the same text the floor draws.
func TestTaskListTreePrintsTheTextTree(t *testing.T) {
	sock, dir := liveServer(t)
	code, out, errb := runCLI(t, sock, dir, "task", "create", "--label", "review-team", "--json")
	if code != 0 {
		t.Fatalf("task create exit %d: %s %s", code, out, errb)
	}
	var res map[string]any
	if err := json.Unmarshal([]byte(out), &res); err != nil || res["task"] != "t1" {
		t.Fatalf("task create --json printed %q", out)
	}
	code, out, errb = runCLI(t, sock, dir, "task", "create", "--label", "docs", "--parent", "t1", "--cwd", t.TempDir())
	if code != 0 {
		t.Fatalf("task create exit %d: %s %s", code, out, errb)
	}
	if !strings.Contains(out, "task t2") {
		t.Fatalf("task create printed %q, want the new task id", out)
	}
	runCLI(t, sock, dir, "pane", "create", "--label", "writer", "--task", "t2", "--", "sh", "-c", "sleep 300")
	code, out, _ = runCLI(t, sock, dir, "task", "list")
	if code != 0 || !strings.Contains(out, "t2") || !strings.Contains(out, "docs") {
		t.Fatalf("task list exit %d printed %q", code, out)
	}
	code, out, errb = runCLI(t, sock, dir, "task", "list", "--tree")
	if code != 0 {
		t.Fatalf("task list --tree exit %d: %s", code, errb)
	}
	for _, want := range []string{"└─ review-team", "└─ docs", "└─ writer"} {
		if !strings.Contains(out, want) {
			t.Fatalf("the tree %q is missing %q", out, want)
		}
	}
	code, _, errb = runCLI(t, sock, dir, "task", "list", "--tree", "--json")
	if code != 1 || !strings.Contains(errb, "--tree takes no other flag") {
		t.Fatalf("task list --tree --json exit %d: %q", code, errb)
	}
	code, out, _ = runCLI(t, sock, dir, "task", "close", "t1")
	if code != 0 || !strings.Contains(out, "closed true") {
		t.Fatalf("task close exit %d printed %q", code, out)
	}
	code, out, _ = runCLI(t, sock, dir, "task", "list", "--tree")
	if code != 0 || !strings.Contains(out, "No tasks.") {
		t.Fatalf("task list --tree on an empty server exit %d printed %q", code, out)
	}
}

// task create --cwd . names the shell's directory, not the server's.
func TestTaskCreateAbsolutizesCwdBeforeSending(t *testing.T) {
	sock, dir := liveServer(t)
	here := t.TempDir()
	t.Chdir(here)
	code, out, errb := runCLI(t, sock, dir, "task", "create", "--label", "x", "--cwd", ".", "--json")
	if code != 0 {
		t.Fatalf("task create exit %d: %s %s", code, out, errb)
	}
	code, out, _ = runCLI(t, sock, dir, "task", "list", "--json")
	if code != 0 {
	...
	if err := json.Unmarshal([]byte(out), &res); err != nil || len(res.Tasks) != 1 {
		t.Fatalf("task list --json printed %q", out)
	}
	want, _ := filepath.Abs(here)
	if res.Tasks[0]["cwd"] != want {
		t.Fatalf("cwd sent = %v, want %s", res.Tasks[0]["cwd"], want)
	}
}

...
var verbSpecs = map[string]map[string]verbSpec{
	...
	"pane": {
		"create": {argv: true, flags: map[string]flagSpec{
			...
			"task":      {wire: "task", kind: "string"},
		}},
		"list": {flags: map[string]flagSpec{
			"ended": {wire: "ended", kind: "bool", value: true},
		}},
		...
		"trust": {pane: true, flags: map[string]flagSpec{
			"not-now": {wire: "trust", kind: "bool", value: false},
		}},
		"run":       {pane: true, trailing: "join", trailingKey: "line"},
		"read":      {pane: true, flags: map[string]flagSpec{"source": {wire: "source", kind: "string"}}},
		"close":     {pane: true},
		// forget takes PANE as a bare positional, or --ended with none;
		// the server refuses when neither is given.
		"forget": {trailing: "join", trailingKey: "pane", flags: map[string]flagSpec{
			"ended": {wire: "ended", kind: "bool", value: true},
		}},
		"resume": {pane: true},
		...
		"explain": {pane: true},
		"fork":    {pane: true, flags: map[string]flagSpec{"label": {wire: "label", kind: "string"}}},
		"rename":  {pane: true, trailing: "join", trailingKey: "label"},
	},
	"task": {
		"create": {flags: map[string]flagSpec{
			"label":    {wire: "label", kind: "string"},
			"parent":   {wire: "parent", kind: "string"},
			"cwd":      {wire: "cwd", kind: "string"},
			"worktree": {wire: "worktree", kind: "bool", value: true},
			"model":    {wire: "model", kind: "string"},
		}},
		"list":        {},
		"close":       {task: true, flags: map[string]flagSpec{"keep-worktree": {wire: "keep_worktree", kind: "bool", value: true}}},
		"move":        {task: true, flags: map[string]flagSpec{"parent": {wire: "parent", kind: "string"}}},
		"set-foreman": {task: true, flags: map[string]flagSpec{"pane": {wire: "pane", kind: "string"}}},
	},
	"agent": {
		"list": {},
		"get":  {pane: true},
		...
		"allow": {pane: true, trailing: "join", trailingKey: "ask", flags: map[string]flagSpec{
			"reason":  {wire: "reason", kind: "string"},
			"confirm": {wire: "confirm", kind: "string"},
			"scope":   {wire: "scope", kind: "string"},
		}},
		"deny": {pane: true, trailing: "join", trailingKey: "ask", flags: map[string]flagSpec{
			"reason": {wire: "reason", kind: "string"},
		}},
	},
	"floor": {
		"note": {trailing: "join", trailingKey: "text"},
		"talk": {trailing: "join", trailingKey: "text"},
	},
	...
	// project add and rm never reach here - runProject answers them
	// itself, straight from the config file, with no socket involved.
	// list is the one project verb the server actually answers.
	"project": {
		"list": {},
	},
}
...
func parseVerbArgs(group, verb string, rest []string) (params map[string]any, asJSON bool, errMsg string) {
	...
	if spec.task {
		if len(positional) == 0 {
			return nil, false, fmt.Sprintf("%s %s needs a task id. Run: coppice task list", group, verb)
		}
		params["task"] = positional[0]
		positional = positional[1:]
	}

...
// COPPICE_DATA_DIR tells the gate hook where this server keeps its
// secrets, so the gate guards them under a custom --data-dir. Only the
// server may name it: an inherited value or a pane's own env never wins.
func TestBuildEnvSetsTheDataDirOverAnyOtherValue(t *testing.T) {
	got := BuildEnv([]string{"PATH=/bin", "COPPICE_DATA_DIR=/inherited"},
		map[string]string{"COPPICE_DATA_DIR": "/from-the-pane"}, "/s", "w1:p1", "/srv/cdata")
	n := 0
	for _, kv := range got {
		if v, ok := strings.CutPrefix(kv, "COPPICE_DATA_DIR="); ok {
			n++
			if v != "/srv/cdata" {
				t.Fatalf("COPPICE_DATA_DIR = %q, want /srv/cdata", v)
			}
		}
	}
	if n != 1 {
		t.Fatalf("env has COPPICE_DATA_DIR %d times, want once: %v", n, got)
	}
}

func TestBuildEnvRemovesAnInheritedDataDirWhenNoneIsGiven(t *testing.T) {
	got := BuildEnv([]string{"PATH=/bin", "COPPICE_DATA_DIR=/inherited"}, nil, "/s", "w1:p1", "")
	for _, kv := range got {
		if strings.HasPrefix(kv, "COPPICE_DATA_DIR=") {
			t.Fatalf("env still carries %q, want the inherited value removed", kv)
		}
	}
}

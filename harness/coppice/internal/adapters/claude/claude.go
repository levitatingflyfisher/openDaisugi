...
func (adapter) Start(ctx context.Context, o pane.StartOpts, _ *pane.Grid) (pane.Proc, error) {
	...
	cmd.Env = pane.BuildEnv(os.Environ(), o.Env, o.Sock, o.PaneID, o.DataDir)
...
func (p *proc) parse(line []byte) []pane.Event {
	...
	if ev, ok := subagentEvent(line); ok {
		return []pane.Event{ev}
	}
...
// subagentHook is the part of a SubagentStart or SubagentStop hook payload
// a child row needs. Claude Code sends agent_id and agent_type in both.
...
// subagentEvent turns a SubagentStart or SubagentStop hook payload into a
// child event: working on start, done on stop, labelled with the agent
// type. ok is false for every other line. A payload with no agent id names
// no child, so it is an error event.
func subagentEvent(line []byte) (pane.Event, bool) {
	var h subagentHook
	...
	case "SubagentStart":
		state = pane.StateWorkingStr
	case "SubagentStop":
		state = pane.StateDoneStr
	default:
		return pane.Event{}, false
	}
	if h.AgentID == "" {
		return pane.Event{Kind: pane.EvError, Detail: h.HookEventName + " with no agent_id"}, true
	}
	return pane.Event{Kind: pane.EvChild, Child: h.AgentID, State: state, Text: h.AgentType}, true
}
...
// Pids names the claude process, so the server can place a process the
// harness starts as a child of this pane. It is empty when there is none.

...
func TestJSUnitTestsPass(t *testing.T) {
	...
	// The view plugins keep their own tests beside them, the shared
	// library's among them.
	views, err := filepath.Glob("../../plugins/*/_tests/*.test.mjs")
	if err != nil {
		t.Fatal(err)
	}
	if len(views) == 0 {
		t.Fatal("no *.test.mjs files found under plugins/*/_tests, so the view tests would not run")
	}

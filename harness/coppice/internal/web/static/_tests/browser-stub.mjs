...
class FakeElement {
  ...
  getAttribute(name) { return this.attrs && name in this.attrs ? this.attrs[name] : null; }
...
// replaceState changes the hash the way a browser does, and fires no
// hashchange.
global.history = { replaceState(_state, _title, url) { if (typeof url === 'string' && url.startsWith('#')) global.location.hash = url; } };
global.location = { origin: 'https://box.example', hash: '', protocol: 'https:' };
Object.defineProperty(global, 'navigator', { value: {}, configurable: true });
global.WebSocket = FakeWebSocket;
global.fetch = (...args) => fetchImpl(...args);
global.CustomEvent = class FakeCustomEvent { constructor(type, init) { this.type = type; this.detail = init ? init.detail : undefined; } };

...
test('a cold start into a blocked pane attaches once the socket opens and shows the ask', async () => {
  ...
  // The recorded ask carries no tier, so it is permanent: no Allow, a name
  // field, and the pane's own label as the name that allows it.
  assert.equal(element('allow').hidden, true);
  assert.equal(element('ask-name').hidden, false);
  assert.ok(element('ask-tier').textContent.includes('type db migration'));
  const calls = [];
  global.window.coppice.api = async (path, options) => {
    calls.push({ path, body: JSON.parse(options.body) });
    return { ok: true };
  };
  element('ask-name').value = 'db migration';
  element('ask-name').dispatchEvent({ type: 'keydown', key: 'Enter' });
  await new Promise((r) => setImmediate(r));
  assert.equal(calls.length, 1);
  assert.equal(calls[0].body.decision, 'allow');
  assert.equal(calls[0].body.confirm, 'db migration');
  assert.equal(calls[0].body.pane, 'w1:p2');

  // A new ask on the same pane starts with an empty name field.
  element('ask-name').value = 'db migration';
  const rows = PANES.map((r) => (r.id === 'w1:p2' ? { ...r, ask: { ...r.ask, id: 'toolu_new' } } : r));
  ws.fire('message', { data: JSON.stringify({ event: 'state', pane: 'w1:p2', ...rows.find((r) => r.id === 'w1:p2') }) });
  await new Promise((r) => setImmediate(r));
  assert.equal(element('ask').dataset.askId, 'toolu_new');
  assert.equal(element('ask-name').value, '', 'the name typed for the last ask stayed');
});
...
test('Allow posts exactly tool_use_id and decision, and clears the box on success', async () => {
  ...
  element('ask').dataset.tier = 'undoable';
...
// A permanent ask on the pane screen allows only with the pane name typed.
// The box starts with no name known, so nothing typed can match, and the
// allow posts nothing until the name is there.
test('the pane screen allows a permanent ask only with the pane name', async () => {
  reset();
  global.localStorage.setItem('coppice.token', TOKEN);
  await freshApp();
  lastSocket().simulateOpen();
  ...
  element('ask').dataset.askId = 'toolu_01ABC';
  element('ask').dataset.tier = 'permanent';
  element('ask').hidden = false;
  element('ask-name').value = 'anything';
  element('allow').dispatchEvent({ type: 'click' });
  element('ask-name').dispatchEvent({ type: 'keydown', key: 'Enter' });
  await new Promise((r) => setImmediate(r));
  assert.equal(calls.length, 0, 'a permanent allow posted without the pane name');
...
// A harness holds this ask itself, so the pane screen answers it through
// coppice-server with agent.deny or agent.allow, never through the gate's
// channel, and a permanent allow still needs the typed name.
test('the pane screen answers a harness-held ask over the operator connection', async () => {
  reset();
  global.localStorage.setItem('coppice.token', TOKEN);
  global.location.hash = '#/pane/w1:p2';
  await freshApp();
  ...
  ws.simulateOpen();
  const paneList = sent.find((m) => m.cmd === 'pane.list');
  const rows = PANES.map((r) => (r.id === 'w1:p2' ? { ...r, ask: { ...r.ask, id: 'per_1', tier: 'permanent', holder: 'harness' } } : r));
  ws.fire('message', { data: JSON.stringify({ id: paneList.id, ok: true, result: { panes: rows } }) });
  await new Promise((r) => setImmediate(r));

  assert.equal(element('ask').hidden, false);
  assert.equal(element('deny').hidden, false);
  assert.equal(element('ask-name').hidden, false);
  assert.ok(element('ask-tier').textContent.includes('The harness holds this ask itself'));
  assert.ok(!element('ask-tier').textContent.includes('coppice agent'));

  const posts = [];
  global.window.coppice.api = async (path, options) => { posts.push({ path, body: options.body }); return { ok: true }; };
  const rpcs = [];
  global.window.coppice.rpc = async (cmd, fields) => { rpcs.push({ cmd, ...fields }); return {}; };
  element('allow').dispatchEvent({ type: 'click' });
  await new Promise((r) => setImmediate(r));
  assert.equal(rpcs.length, 0, 'a permanent allow went out with no name');
  element('deny').dispatchEvent({ type: 'click' });
  await new Promise((r) => setImmediate(r));
  assert.equal(posts.length, 0, 'a held ask posted to the gate channel');
  assert.deepEqual(rpcs, [{ cmd: 'agent.deny', pane: 'w1:p2', ask: 'per_1', reason: 'denied from the phone' }]);
});

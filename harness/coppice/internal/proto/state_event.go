...
// The tiers an ask can carry. An allowed call never asks, so an ask is
// undoable or permanent. Every reader treats a missing or unknown tier as
// permanent.
const (
...
type Ask struct {
	...
	// Holder is HolderHarness when a headless harness holds the ask itself,
	// so the gate cannot answer it and only agent.allow and agent.deny
	// can. Only the server sets it.
	Holder string `json:"holder,omitempty"`
}

// HolderHarness marks an ask a headless harness holds itself.
const HolderHarness = "harness"

// NormalTier is TierUndoable for exactly "undoable" and TierPermanent for
// anything else, the empty string included.
func NormalTier(tier string) string {
...
type PaneStateEvent struct {
	...
	// Who names the person behind an operator event. The server sets it
	// from the connection's name. It is nil on every other event.
...
func (e PaneStateEvent) Validate() error {
	...
	// An ask with no tier, or a tier nobody defined, reads as permanent.
	// Ask is a pointer, so this sets the tier on the event the caller holds.
	if e.Ask != nil {
		e.Ask.Tier = NormalTier(e.Ask.Tier)
	}
	...
	if e.Who != nil && e.Source != SrcOperator {
		return fmt.Errorf("who is only valid on an operator event, source is %q", e.Source)
	}
	if e.V != 1 {
...
func ParseStateEvent(line []byte) (PaneStateEvent, error) {
	...
	who, err := optionalStringFromRow(row, "who")
	if err != nil {
		return PaneStateEvent{}, err
	}
...
// and deadline must be a number. A bool or a null deadline is refused.
// tier is optional.
func parseAsk(raw json.RawMessage) (*Ask, error) {
	...
	// tier is optional. Absent or JSON null reads as permanent. A present
	// value that is not a string is an error, the same as the other fields.
	tier := ""
	if raw, ok := obj["tier"]; ok && !isJSONNull(raw) {
		tier, err = stringFromRaw(raw, "ask tier")
		if err != nil {
			return nil, err
		}
	}
	return &Ask{ID: id, Tool: tool, Summary: summary, Deadline: deadline, Tier: NormalTier(tier)}, nil
}

// TrustRule is the claude.toml rule that sees Claude Code's folder trust
// screen. The manifest tick names it in a state event's detail as
// "rule=coppice_first_run_trust region=...".
const TrustRule = "coppice_first_run_trust"

// IsTrustDetail reports whether a state event's detail came from the
// trust rule, so a client can offer "Trust this folder" and "Not now".
func IsTrustDetail(detail string) bool {
	return strings.Contains(detail, "rule="+TrustRule+" ")
}

...
func TestTheFourWordBoundaryOnVerbs(t *testing.T) {
	...
	if Classify("open claude --model opus --verbose", h) != Talk {
	...
	if Classify("please open claude", h) != Talk {
...
func TestHarnessesAndVerbsAreTheAgreedLists(t *testing.T) {
	...
	if strings.Join(Verbs, " ") != "list read close open swap rotate reset lock unlock layout" {
		t.Fatalf("Verbs = %v", Verbs)
	}
}

func TestNoForemanNamesTheDefaultHarness(t *testing.T) {
	got := NoForeman("pi")
	want := `No foreman. Enter opens pi here, or type "foreman <harness>" to let it run the floor.`
	if got != want {
		t.Fatalf("got %q\nwant %q", got, want)
	}
}

...
// 1-based. It is false for a column outside every tile. It reads the
// column only. OnTiles tells whether a screen row holds the tiles.
...
// OnTiles reports whether the tiles are drawn on screen row y, 1-based.
// The bar, the message line, the footer and the prompt are not.
func OnTiles(m *Model, y int) bool {
	return y >= 1 && y <= len(m.TileY) && m.TileY[y-1]
}

...
func TestWindowCountFitsReadableWindowsBesideTheRail(t *testing.T) {
	for cols, want := range map[int]int{80: 0, 118: 0, 120: 1, 199: 1, 200: 2, 280: 2, 290: 3, 400: 4} {
		if got := windowCount(cols); got != want {
			t.Fatalf("windowCount(%d) = %d, want %d", cols, got, want)
		}
	}
}

func TestEveryWindowIsAtLeastEightyColumns(t *testing.T) {
	for cols := 100; cols <= 420; cols++ {
		n := windowCount(cols)
		if n == 0 {
			continue
		}
		rail, widths := splitWidths(cols, n)
		sum := rail
		for _, w := range widths {
			if w < minWindow {
				t.Fatalf("at %d columns a window is %d wide", cols, w)
			}
			sum += w + 1
		}
		if sum != cols {
			t.Fatalf("at %d columns the parts add to %d", cols, sum)
		}
	}
}
...
func rightOfDivider(lines []string) []string {
	var out []string
	for _, l := range lines {
		parts := strings.Split(strings.ReplaceAll(stripSGR(l), accentDivider, divider), divider)
		if len(parts) > 1 {
...
func TestAWideScreenSplitsAndTheTileCarriesAHeader(t *testing.T) {
	...
	if !strings.Contains(stripSGR(strings.Join(lines, "\n")), "  › 1 gate-refactor") {
		t.Fatal("the rail lost its cursor row or its window number")
	}
}
...
func TestTwoTilesAt200Columns(t *testing.T) {
	...
	for _, l := range lines[1:] {
		if strings.Count(l, "│") != 2 && strings.TrimSpace(stripSGR(l)) != "" && !strings.Contains(l, Keys()) &&
...
func TestAnEmptySlotSaysWhatToDo(t *testing.T) {
	...
	if !strings.Contains(right, emptySlotText) {
...
// A wide floor opens with the agent live in its window and the keys
// there, with no key pressed.
func TestAWideFloorOpensWithTheAgentLiveInAWindow(t *testing.T) {
	s := newTestServer(t)
...
type recordingProxy struct {
	...
	// way a slow server would. holdSize does the same for every tile
	// attach and resize reply.
	holdDetach time.Duration
	holdSize   time.Duration
}
			...
			go func() {
				...
				for {
					...
					if len(line) > 0 {
						...
						if p.holdSize > 0 && (strings.Contains(string(line), `"id":"va-`) || strings.Contains(string(line), `"id":"vr-`)) {
							time.Sleep(p.holdSize)
						}
						p.log("<" + string(line))
...
func TestAReattachWaitsForTheDetachReply(t *testing.T) {
	...
	for attempt := 0; attempt < 5; attempt++ {
		...
		_, _ = io.WriteString(pw, "\x1b[<0;3;"+strconv.Itoa(yb)+"M\x1b[<0;3;"+strconv.Itoa(yb)+"m\x1b[<0;3;"+strconv.Itoa(ya)+"M\x1b[<0;3;"+strconv.Itoa(ya)+"m")

...
```bash
daisugi start                # → a working shadow-mode gate in minutes
```
...
`daisugi tiers setup` to detect your hardware and wire a right-sized **local** model as a
free-ish Tier-1 (only if it passes a qualification gate). Not sure where to start?
`daisugi start` gates this directory and opens the live view in one command.
...
CLI tree: bare `daisugi` or `daisugi help` for the start-here text, `daisugi help --all`
for every command grouped, `--help` at each level. The visible top level is ten
commands in five panels — Start here (`start`, `status`, `dashboard`, `help`), Run
(`orchestrate`), Gate (`gate`, `install`), Garden (`pathways`, `journal`), Settings
(`config`) — plus operational groups and commands that still run and are still
suggested on a typo, just not shown by default: `onboard`, `run`, `verify`, `tend`,
`route`, `generate-envelope`, `gateway`, `viz`, `models`, `metrics`; subcommand groups
`tiers`, `gardener`, `lora`, `mcp`, `hook`, `registry`, `release`, `batch`, `conformance`.
